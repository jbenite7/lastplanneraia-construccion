...
class CicApiController
{
    ...
    public function list(): void
    {
        ...
        try {
            ...
            for ($s = 1; $s <= $semana; $s++) {
                $conteo = $this->db->query("SELECT COUNT(*) FROM {$dbPrefix}_cic WHERE Semana = ?", [$s])->fetchColumn();
                if ($conteo > 0) {
            ...
            $subs = $this->db->query("SELECT DISTINCT(subcontratista) FROM {$dbPrefix}_cic WHERE Semana <= ? {$filter} ORDER BY subcontratista ASC", [$semana])->fetchAll(\PDO::FETCH_COLUMN);
    ...
    {
        $subs = $this->db->query("SELECT DISTINCT Sub_Contratista FROM {$db}_programacion_semanal WHERE Semana=? AND Sub_Contratista !='' AND (Activa=1 OR Activa='NA')", [$s])->fetchAll(PDO::FETCH_COLUMN);
        foreach ($subs as $sub) {
            $stats = $this->db->query("SELECT ROUND(AVG(P_Completado),3) as P_Com, ROUND(AVG(PAC),3) as PAC FROM {$db}_programacion_semanal WHERE Semana=? AND Sub_Contratista =? AND (Activa=1 OR Activa='NA')", [$s, $sub])->fetch(PDO::FETCH_ASSOC);
            $this->db->query("UPDATE {$db}_cic SET P_Completado = ?, PAC = ? WHERE subcontratista = ? AND Semana = ?", [$stats['P_Com'] ?? 0, $stats['PAC'] ?? 0, $sub, $s]);
        }
    }
    ...
    private function updateMetrics(string $db, $id, $semana, string $type): void
    {
        ...
        $sql = "UPDATE {$db}_cic SET " . implode(', ', $fields) . ", Observaciones = ? WHERE Id = ?";
    ...
    {
        $rows = $this->db->query("SELECT * FROM {$db}_cic WHERE Semana = ?", [$s])->fetchAll(\PDO::FETCH_ASSOC);
        foreach ($rows as $cic) {
            $id = $cic['Id'];
            ...
            $sql = "SELECT 
                (SELECT CASE WHEN (SELECT COUNT(*) FROM {$db}_cic WHERE Semana<=? AND subcontratista=? AND PAC!='NA')=0 THEN NULL ELSE (SELECT ROUND(AVG(PAC),3) FROM {$db}_cic WHERE Semana<=? AND subcontratista=? AND PAC!='NA') END) AS PAC_Acum,
                (SELECT CASE WHEN (SELECT COUNT(*) FROM {$db}_cic WHERE Semana<=? AND subcontratista=? AND P_Completado!='NA')=0 THEN NULL ELSE (SELECT ROUND(AVG(P_Completado),3) FROM {$db}_cic WHERE Semana<=? AND subcontratista=? AND P_Completado!='NA') END) AS P_Completado_Acum,
                (SELECT CASE WHEN (SELECT COUNT(*) FROM {$db}_cic WHERE Semana<=? AND subcontratista=? AND Calidad!='NA' AND Calidad!='NR')=0 THEN 'NA' ELSE (SELECT ROUND(AVG(Calidad),3) FROM {$db}_cic WHERE Semana<=? AND subcontratista=? AND Calidad!='NA' AND Calidad!='NR') END) AS Calidad_Acum,
                (SELECT CASE WHEN (SELECT COUNT(*) FROM {$db}_cic WHERE Semana<=? AND subcontratista=? AND GSA!='NA' AND GSA!='NR')=0 THEN 'NA' ELSE (SELECT ROUND(AVG(GSA),3) FROM {$db}_cic WHERE Semana<=? AND subcontratista=? AND GSA!='NA' AND GSA!='NR') END) AS GSA_Acum,
                (SELECT CASE WHEN (SELECT COUNT(*) FROM {$db}_cic WHERE Semana<=? AND subcontratista=? AND SST!='NA' AND SST!='NR')=0 THEN 'NA' ELSE (SELECT ROUND(AVG(SST),3) FROM {$db}_cic WHERE Semana<=? AND subcontratista=? AND SST!='NA' AND SST!='NR') END) AS SST_Acum,
                (SELECT CASE WHEN (SELECT COUNT(*) FROM {$db}_cic WHERE Semana<=? AND subcontratista=? AND ADM!='NA' AND ADM!='NR')=0 THEN 'NA' ELSE (SELECT ROUND(AVG(ADM),3) FROM {$db}_cic WHERE Semana<=? AND subcontratista=? AND ADM!='NA' AND ADM!='NR') END) AS ADM_Acum";
            ...
            $row2 = $this->db->query("SELECT * FROM {$db}_cic WHERE Id=?", [$id])->fetch(\PDO::FETCH_ASSOC);
            ...
            $this->db->query("UPDATE {$db}_cic SET Cal_Integral=?, Cal_Integral_Acum=? WHERE Id=?", [$cal, $calAcum, $id]);
        }
    }
