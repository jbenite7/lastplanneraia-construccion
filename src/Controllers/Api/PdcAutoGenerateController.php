...
class PdcAutoGenerateController
{
    ...
    /**
     * Auto-asigna paquetes a las actividades usando el motor de sugerencias.
     * Ejecuta el pipeline completo: cargar PG → matchear familias → seleccionar opciones → escribir actividades.
     * No requiere interacción del usuario (auto-selecciona por confianza y estrategia guardada).
     *
     * @return array{asignadas: int, conflictos: array, omitidas: int}
     */
    private function autoAssignToActividades(string $dbPrefix, int $semana): array
    {
        $totalAsignadas = 0;
        $totalConflictos = [];
        $totalOmitidas = 0;
        ...
        foreach ($groups as $familyId => $group) {
            ...
            $option = $this->loadOption($selectedOptionId);
            if ($option === null) {
                continue;
            }

            $fechaInicio = $this->normalizeDate($group['fechaInicio']);
            $suggestion = [
            ...
            $totalAsignadas += $wbResult['escritas'];
            $totalOmitidas += $wbResult['omitidas'];
            if (!empty($wbResult['conflictos'])) {
                $totalConflictos = array_merge($totalConflictos, $wbResult['conflictos']);
            }
        }

        return ['asignadas' => $totalAsignadas, 'conflictos' => $totalConflictos, 'omitidas' => $totalOmitidas];
    }
    ...
    public function applyFromActividades(): void
    {
        try {
            ...
            // Fase 1: Auto-asignar paquetes a las actividades (write-back)
            $autoAssign = $this->autoAssignToActividades($dbPrefix, $semana);
            ...
            $this->jsonResponse([
                ...
                'autoAsignadas' => $autoAssign['asignadas'] ?? 0,
                'autoConflictos' => $autoAssign['conflictos'] ?? [],
            ]);
