...
try {
    $stmt = $dbInstance->queryWithProject("SELECT COUNT(*) AS conteo FROM {$tSemanasActivas}");
    $data = $stmt->fetch();
    $conteo = (int) ($data["conteo"] ?? 0);

    $semanalConfirmada = 0;
    if ($conteo > 0) {
        $stmtVerif = $dbInstance->queryWithProject("SELECT Semanal_Confirmada FROM {$tSemanasActivas} WHERE Semana = ?", [$conteo]);
    ...
    if ($conteo > 0 && $semanalConfirmada == 0 && !$esAdmin) {
        ...
        if ($conteo == 0) {
            ...
            $stmtPrograma = $dbInstance->queryWithProject("SELECT COUNT(*) AS c FROM {$tPrograma}");
            if ((int) $stmtPrograma->fetch()['c'] === 0) {
                // Rollback: eliminar la semana recién creada
                $dbInstance->queryWithProject("DELETE FROM {$tSemanasActivas} WHERE Semana = ?", [$semana_crear]);
            ...
            $dbInstance->queryWithProject($sqlReset, [$semana_crear]);

        } else {
            $stmtMax = $dbInstance->queryWithProject("SELECT MAX(Semana) AS max_semana FROM {$tProgConsolidado}");
            ...
            if ($maxSemanaProgramaConsolidado > $semana_crear) {
                $dbInstance->queryWithProject("DELETE FROM {$tProgConsolidado} WHERE Semana > ?", [$semana_crear]);
            ...
            if ($maxSemanaProgramaConsolidado == $semana_crear) {
                ...
                $sqlBigUpdate = "UPDATE {$tProgConsolidado} AS dest
                                 INNER JOIN (SELECT * FROM {$tProgConsolidado} WHERE Semana = ? AND Titulo = 0) AS src
                                 ON REPLACE(REPLACE(dest.programaAnteriorAsociar, '<b>', ''), '</b>', '') = REPLACE(REPLACE(src.Actividad, '<b>', ''), '</b>', '')
                                 SET $setClause
                                 WHERE dest.Semana = ?";
                $dbInstance->queryWithProject($sqlBigUpdate, [$conteo, $semana_crear]);

                $sqlReprog = "UPDATE {$tSemanasActivas} SET reprogramacion=1, diferenciaEstructuraCron=(SELECT COUNT(*) FROM {$tProgConsolidado} WHERE Semana=? AND Fecha_Inicio IS NOT NULL AND Fecha_Fin IS NOT NULL AND Titulo != 1 AND (Ejecutado IS NULL OR Estado_Restricciones IS NULL OR programaAnteriorAsociar IS NOT NULL)) WHERE Semana=?";
                $dbInstance->queryWithProject($sqlReprog, [$semana_crear, $semana_crear]);
            ...
            if ($isPreConstruccion) {
                ...
                $dbInstance->queryWithProject("UPDATE {$tProgConsolidado} SET Ejecutado = 0, Estado_Restricciones = '0', restriccion_pc_1 = '0%', restriccion_pc_2 = '0%', restriccion_pc_3 = '0%', restriccion_pc_4 = '0%' WHERE Ejecutado IS NULL AND Semana = ? AND Titulo=0", [$semana_crear]);
            } else {
                $dbInstance->queryWithProject("UPDATE {$tProgConsolidado} SET Semanas_Inicio=NULL, medir_productividad=NULL, unidad=NULL, cantidad_ppto=NULL, codigo_actividad=NULL, D_y_E='0', Materiales='0', MdeO='0', Equipos='0', Predecesora='0', Pdto_Cons='0', Modelo='0', Sub_Contratista=NULL, Responsable_AIA=NULL, Observaciones=NULL, Ult_Act_Est=NULL, Ult_Act_Restr=NULL, Activa=0 WHERE Semana = ? AND Titulo=1", [$semana_crear]);
                $dbInstance->queryWithProject("UPDATE {$tProgConsolidado} SET Ejecutado = 0, Estado_Restricciones = '0', D_y_E = '0', Materiales = '0', MdeO = '0', Equipos = '0', Predecesora = '0', Pdto_Cons = '0', Modelo = '0' WHERE Ejecutado IS NULL AND Semana = ? AND Titulo=0", [$semana_crear]);
            }
