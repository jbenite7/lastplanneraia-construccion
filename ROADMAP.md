...
- [x] **Fix Color PG con Filtro + Scroll (2026-06-10):** Se agregó hook `afterLoadData` en `hot.js` de Programa General para invalidar el cache de clasificación y refrescar el cellMeta al recargar datos. Esto previene que el cache quede desincronizado cuando Handsontable reemplaza el source data completo. Test E2E `test-pg-color-fix.mjs` valida que filas filtradas mantienen clases de estado correctas tras scroll a 8000px.
