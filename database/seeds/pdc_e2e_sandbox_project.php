...
// Sube aqui, con las demas constantes: el cuerpo ejecutable del seed vive ahora dentro de una
// closure (SystemScopeRunner) y PHP no admite `const` dentro de una funcion.
/**
...
// ProjectSqlGuard (2026-08-29) exige un alcance activo para tocar tablas con project_id, y este
// seed no pasa por SessionMiddleware: lo invocan los specs por linea de comandos. El alcance que
// le corresponde es SystemScope, no ProjectScope, y no por comodidad: `pdcSandboxLimpiarGlobales()`
// borra en `general_paquetes_contratacion` (tabla System, sin project_id) uniendo contra tablas de
// proyecto para no llevarse dato ajeno. Bajo un ProjectScope el guard exige que la RAIZ lleve su
// `project_id = ?`, y esa raiz no tiene la columna: no hay forma de expresar ese borrado con el
// alcance de una obra. SystemScopeRunner es la puerta auditada para eso, y por eso este archivo
// esta en la lista de tests/test_project_scope_callsite_audit.php: el permiso queda a la vista.
//
// El `exit(0)` de la rama --purge termina el proceso sin pasar por el `finally` que limpia el
// alcance. Da igual aqui —el proceso muere con el— pero no se copie el patron a codigo servido.
(new SystemScopeRunner($db->dataScope()))->run(
    'seed del sandbox e2e del Plan de Compras (database/seeds/pdc_e2e_sandbox_project.php)',
    static function () use ($db, $purge): void {
