...
| 2026-08-14 | Fase 2 (tarjeta) | **El contador del chip cuenta las 5 restricciones duras en los dos módulos** (`E2-bis-e`); las 2 blandas siguen editables al desplegar pero no cuentan como bloqueo | La asimetría no era entre módulos sino dentro de Intermedia: `isReadyToCommit()` decide con las duras, así que «3 de 7» habría mostrado un número distinto al que determina el estado de su propia tarjeta. Queda escrita la discrepancia con `GLOSARIO.md:47`, que habla de siete: si el criterio de obra es ese, lo que cambia es la regla, no el contador |
| 2026-08-14 | Fase 2 (tarjeta) | **Las dos tarjetas adoptan una forma común: cifra accionable en el chip, línea de qué falta, desplegable nombrado por su contenido** (`E2-bis-d`). El capítulo se separa del título con `extractChapter()`, y la frase de foco cambia con la fase de la semana | Comparar las dos maquetas mostró que Intermedia era mejor por responder «qué hago» en vez de «qué es». `focus` ya se calculaba y solo lo recibía el lector de pantalla: publicarlo es exponer dato existente |
| 2026-08-14 | Fase 2 (tarjeta) | **Las siete restricciones de Intermedia no están en su tarjeta móvil, y son el núcleo del módulo.** Decisión: resumen visible («3 de 7») y edición en el desplegable (`E2-bis-c`) | Lo destapó una pregunta del usuario al ver la maqueta. El análisis previo contó los siete campos presentes y concluyó que cabían todos, sin comprobar si faltaba algo. Arrastra que la tarjeta de Intermedia pase de solo lectura a editable, atada a las reglas I1–I7 ya extraídas |
