...
| D65 | Atribución en las causas | **La gráfica deja de truncar el sufijo de atribución** (obra / subcontratista) | No eran duplicados: son tres causas distintas y el truncamiento borra el dato más político del tablero. La tercera variante, sin atribuir, sí es deuda de catálogo. **Corregido 2026-08-20 al implementar F0 (Task 3):** el truncamiento se originó en el tooltip de Power BI, no en `bi-spa.js` — verificado en BD y navegador que la Torre ya mostraba el texto completo; se añadió `title` como defensa y una prueba de regresión real. D65 queda como contrato de la Torre hacia adelante |
...
| D74 | Entrevistas con la obra | ~~Un residente y un director, esta semana~~ · **Revocada el 2026-08-20 por D92** | — |
| D92 | **Se avanza sin entrevistar a residente ni director** | Felipe decidió no hacer las dos conversaciones. **Riesgo aceptado, no pendiente**; deja de ser condición de cierre de F1 | Se hizo una entrevista, con Felipe en ambos roles, y él mismo diseñó el tablero: no es validación independiente. D36 y D91 quedan en entredicho sin resolver, y D86 contra la evidencia disponible. **La validación se traslada al comportamiento medido**: el criterio de muerte a 90 días y la respuesta al correo a las 4 semanas. Si no se mueven, la hipótesis era falsa — y se sabrá habiendo construido F1 y F2, no habiendo preguntado veinte minutos |
...
| D82 | Historial de compras de JMC y Milán | **No existe historial de ejecución en ninguna parte.** Ambas obras **deben construir su plan en Last Planner AIA, y está pendiente** | Las 273 filas vacías son plantillas esperando a que alguien las llene |
| D85 | **La memoria de duraciones sí es vigente** | Corrección de Felipe. AIA **sí tiene** memoria de cuánto tarda contratar: `general_dias_procesos_contratacion`, **216 paquetes con siete duraciones cada uno**, más `duracion_ref` en 169 de 221 paquetes. **43 de los 62 paquetes de Da Porto traen la suya propia** | La sesión principal había escrito que «la calibración arranca de cero», confundiendo **no estar medido en la base** con **no existir**. El conocimiento vive en la empresa y está codificado: ascensores 300 días de fabricación, carpintería 90, baños portátiles 0. Lo que falta no es la memoria: es **cerrar el lazo** entre lo planeado y lo ocurrido. El contador de D80 sirve para **calibrar** el catálogo, no para construirlo |
