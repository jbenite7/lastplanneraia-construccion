...
- **Fase A3 — Paquetes de contratación + motor de sugerencias** *(spec `docs/superpowers/specs/2026-07-23-a3-paquetes-contratacion-design.md`)*: `general_paquetes_contratacion` (catálogo global) + `pdc_insumo_paquete` (asignación por `project_id`, un-insumo-un-paquete, atada a la versión activa). Los 4 tipos de negociación como atributo del paquete. **Motor de sugerencias cross-proyecto sin tabla nueva** (agrega sobre `pdc_insumo_paquete`): exacto por insumo, similitud por tokens, y Agrupación SINCO como filtro + señal de respaldo; siempre con confirmación humana. Asignación masiva en AG Grid; indicador **100% asignado**; RBAC `lps.paquetes_contratacion.*` (ya en catálogo).
...
Dependencias: A1→A2→A2.5→A3→A4→B1→B2; B3 tras B1; C1 al final. La UI de la SPA se organiza desde A1 con la navegación de los 2 submódulos (Ensamble | Seguimiento).
