...
| 6–7 · Shell | Riel, iconos, contraste, estados y selector de semana en la barra de contexto, en ambos temas. | `32b15bbb`, `e77540a2` |
...
| 8 · Pruebas | Persistencia React UI→API→BD, restauración, RBAC real, presupuesto de runtime y flujo móvil con el disparador real. | `d8459b2b`, `c9ebacfa`, `784f846f`, `a87c8824`, `f942d76e` |
...
| `npm run test:runtime-budget:measure` y `npm run test:runtime-budget:check` | Pendiente de corrida final con worktree limpio |
| `programa-general.visual.mjs` contra referencias previas | RC 1 esperado · dos diferencias visuales; referencia nueva sin aprobación |
...
- **BLOCKED · tarea 9:** Felipe rechazó las capturas iniciales. Las seis imágenes de referencia, candidata y diferencia en dos tamaños están en `docs/superpowers/evidence/s05-ronda-1-2/goldens-candidatas/`. Se solicitó un nuevo visto sobre las candidatas posteriores a R1.2-5. Hasta obtenerlo, no se modifican los goldens ni se considera verde el gate visual.
