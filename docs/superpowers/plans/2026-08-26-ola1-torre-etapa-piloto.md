...
**34 entradas en la Bitácora**, Tasks 1 a 9 de esta etapa (el contador corre desde la Tarea 1, regla
...
| Hallazgo (puro o compuesto) | 20,27,29,31,32,33,34 | 7 |
| Corrección | 22,26 | 2 |
| Confirmación | 21 | 1 |
| Ganancia | 3 | 1 |
| Bloqueo (gate) | 31 | 1 |
...
| 34 | Hallazgo + fix (scroll roto en toda la hoja) | Felipe, revisando el piloto ya publicado, reportó no poder hacer scroll en `/bi/intermedia`. Reproducido: `getComputedStyle(document.documentElement).overflowY` daba `hidden` con `scrollHeight` de ~4200px contra un `clientHeight` de 820px — ningún elemento de la cadena (`html`, `body`, `#root`, `.ct-intermedia-layout`) tenía scroll propio. Systematic-debugging (Fase 1, evidencia antes que hipótesis): recorrer `document.styleSheets` a mano reveló que `aia-design-system.css` importa `handsontable-module.css` (compartido por las 8 hojas viejas del BI), que fija a propósito `html, body { overflow: hidden }` en `@layer base` — comentario propio del archivo ya avisaba "Handsontable handles scrolls on Desktop": las tablas viejas manejan su scroll interno y el documento no necesita el suyo. `ct-app` no usa Handsontable, su contenido excede el viewport, y hereda el bloqueo sin necesitarlo — nadie lo había visto porque ninguna hoja nueva sin tabla había consumido ese CSS compartido hasta este piloto. Confirmado que el sidebar no depende de ese `overflow:hidden` para quedarse fijo: usa su propio `position: fixed` (`shell-sidebar.css`), así que restaurar el scroll del documento no lo mueve. Fix acotado a `ct-app/src/pages/Intermedia.css` (una regla `html, body { height: auto; overflow-y: auto; }`, sin capa — Vite no envuelve el bundle en `@layer`, así que gana automáticamente sobre la regla de `handsontable-module.css` sin `!important`); no toca el CSS compartido ni las 8 hojas viejas. Reverificado: scroll real del documento funciona (`window.scrollBy` mueve `scrollY`), 3/3 del e2e, 155/155 Vitest de `ct-app`, 8/8 del suite estático (el sha256 del golden ya generado no cambió: el cambio es de comportamiento de overflow, no de renderizado visual). | Ninguno sobre datos de producción ni sobre las 8 hojas viejas (el fix vive solo en el bundle de `ct-app`, que ellas no cargan). El costo si hubiese quedado sin corregir: la hoja piloto era literalmente inutilizable con más de una pantalla de restricciones — el caso real de cualquier obra con datos, no el sandbox vacío que probó el e2e. Publicado en
[PR #16](https://github.com/jbenite7/lastplanneraia-construccion/pull/16), mergeado a `main`
(`8e69964b159c9b240db8bf48056729346279129b`) con CI en verde sobre el commit exacto verificado
(`design-system-static` 1m48s, `design-system-runtime` 10m0s, ambos `pass`). `git rev-parse
origin/main` confirma el mismo SHA tras el fetch posterior al merge. |
