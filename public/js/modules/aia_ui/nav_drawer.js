(function (global) {
  ...
  function init(root) {
    ...
    function openDrawer() {
      ...
      global.setTimeout(function () { ui.close.focus(); }, 450);
    }
    ...
    function onKeydown(event) {
      ...
      if (event.key === 'Tab') {
        var items = Array.from(ui.drawer.querySelectorAll(focusableSelector)).filter(isVisibleFocusable);
    ...
    ui.close.addEventListener('click', function () { closeDrawer(true); });
    ui.overlay.addEventListener('click', function () { closeDrawer(true); });
