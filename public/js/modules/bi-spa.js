...
function renderCausalDrilldownTable(rows, type) {
  ...
  body.innerHTML = rows.map((row) => `<tr>
    ...
    <td><strong>${escapeHtml(row.category || '--')}</strong><br><small title="${escapeHtml(row.cause || '--')}">${escapeHtml(row.cause || '--')}</small><br><small>${escapeHtml(row.observation || 'Sin observación')}</small></td>
...
function renderCausalField(label, value) {
  const text = escapeHtml(String(value || '').trim() || 'Sin asignar');
  return `<div><dt>${escapeHtml(label)}</dt><dd title="${text}">${text}</dd></div>`;
}
