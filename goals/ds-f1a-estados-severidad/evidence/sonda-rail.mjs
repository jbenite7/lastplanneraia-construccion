...
const datos = await page.evaluate(() => {
  ...
  for (const tr of document.querySelectorAll('#hot-container .ht_master tbody tr')) {
    ...
    filas0.push({
      estadoTr: tr.getAttribute('data-aia-severity-rail'),
      clase: (tr.className.match(/pi-state-[\w-]+/) || ['?'])[0],
      td0attr: td0 ? td0.getAttribute('data-aia-severity-rail') : 'SIN-TD',
      td0shadow: td0 ? getComputedStyle(td0).boxShadow : '-',
      td0clase: td0 ? td0.className.slice(0,60) : '-',
    });
  }
  window.__filas0 = filas0;
...
console.log(JSON.stringify(await page.evaluate(()=>window.__filas0),null,1));
writeFileSync(`${OUT}medicion-rail.json`, JSON.stringify(salida, null, 2));
await page.screenshot({ path: `${OUT}pi-filete-apagado-1180x820-dark.png` });
