...
$query2="INSERT INTO general_curvas (`Proyecto`, `fInicioProyecto`, `fFinProyecto`, `Semana`, `Fecha_Inicio_Sem`, `Fecha_Fin_Sem`,  `diasCompletadosReal`, `diasCompletadosTeorico`, `diasCompletadosLineaBase`, `diasTotales`, `porcentajeCompletadoReal`, `porcentajeCompletadoTeorico`)";
while ($data1=mysqli_fetch_assoc($resultado1)){
    ...
    while ($data3=mysqli_fetch_assoc($resultado3)){
      ...
      LEFT JOIN (SELECT $Semana AS Semana, SUM((SELECT
      CASE
      WHEN (DATEDIFF('$Fecha_Inicio_Sem', `Fecha_Inicio`)+1) <= 0 THEN 0
      WHEN (DATEDIFF('$Fecha_Inicio_Sem', `Fecha_Inicio`)+1) >= (DATEDIFF(`Fecha_Fin`, `Fecha_Inicio`)+1) THEN (DATEDIFF(`Fecha_Fin`, `Fecha_Inicio`)+1)
      ELSE (DATEDIFF('$Fecha_Inicio_Sem', `Fecha_Inicio`)+1)
      END)) AS diasCompletadosLineaBase, 
      (SELECT SUM(DATEDIFF(`Fecha_Fin`, `Fecha_Inicio`)+1) FROM $Base_de_Datos"."_programa_consolidado WHERE `Semana`=1 AND `Titulo`=0) AS diasTotalesLineaBase 
      FROM `$Base_de_Datos"."_programa_consolidado` WHERE Semana=1 AND Titulo=0) AS tablaLineaBase 
      ON $Base_de_Datos"."_programa_consolidado.`Semana` = tablaLineaBase.`Semana`
    ...
    for ($i=($Semana+1); $i < ($semanasProyecto+1); $i++) {
      ...
      (DATEDIFF($Base_de_Datos"."_programa_consolidado.`Fecha_Fin`, $Base_de_Datos"."_programa_consolidado.`Fecha_Inicio`)+1) AS diasTotalesActividad, 
      (SELECT SUM(DATEDIFF($Base_de_Datos"."_programa_consolidado.`Fecha_Fin`, $Base_de_Datos"."_programa_consolidado.`Fecha_Inicio`)+1) FROM $Base_de_Datos"."_programa_consolidado WHERE $Base_de_Datos"."_programa_consolidado.`Semana`=$Semana AND $Base_de_Datos"."_programa_consolidado.`Titulo`=0) AS diasTotales
      FROM $Base_de_Datos"."_programa_consolidado

      LEFT JOIN (SELECT $i AS Semana, SUM((SELECT
      CASE
      WHEN (DATEDIFF('$Fecha_Inicio_Sem', `Fecha_Inicio`)+1) <= 0 THEN 0
      WHEN (DATEDIFF('$Fecha_Inicio_Sem', `Fecha_Inicio`)+1) >= (DATEDIFF(`Fecha_Fin`, `Fecha_Inicio`)+1) THEN (DATEDIFF(`Fecha_Fin`, `Fecha_Inicio`)+1)
      ELSE (DATEDIFF('$Fecha_Inicio_Sem', `Fecha_Inicio`)+1)
      END)) AS diasCompletadosLineaBase, 
      (SELECT SUM(DATEDIFF(`Fecha_Fin`, `Fecha_Inicio`)+1) FROM $Base_de_Datos"."_programa_consolidado WHERE `Semana`=1 AND `Titulo`=0) AS diasTotalesLineaBase FROM `$Base_de_Datos"."_programa_consolidado` WHERE Semana=1 AND Titulo=0) AS tablaLineaBase 
      ON $Base_de_Datos"."_programa_consolidado.`Semana` = tablaLineaBase.`Semana`


      LEFT JOIN $Base_de_Datos"."_semanas_activas
      ON $Base_de_Datos"."_programa_consolidado.`Semana` = $Base_de_Datos"."_semanas_activas.`Semana`
      WHERE $Base_de_Datos"."_programa_consolidado.`Titulo`=0 AND $Base_de_Datos"."_programa_consolidado.`Fecha_Inicio` IS NOT NULL AND $Base_de_Datos"."_programa_consolidado.`Fecha_Fin` IS NOT NULL AND $Base_de_Datos"."_programa_consolidado.`Semana`=$i) AS tabla GROUP BY tabla.`Semana` UNION ";
