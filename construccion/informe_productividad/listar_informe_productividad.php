...
// $opcion="Seguimiento_Cuadrillas_Tipicas";
// $semana=24;
// $codigo_actividad="0000021";
...
switch($opcion){
    ...
    // $oficiales_teorico=9;
    // $ayudantes_teorico=0;
    // $cuadrilla_tipica_teorico=3;
    // $rendimiento_cuadrilla_tipica_teorico=833;
    ...
    // $oficiales_tipica = 1;
    // $ayudantes_tipica = 2;
    // $rendimiento_tipica = 1.5;
    // $numero_cuadrillas_tipicas = 5;
...
function grafico_Seguimiento_Ejecucion_Proyectado($conexion, $semana, $db, $codigo_actividad){
    ...
    if(!$resultado){
        ...
        $query2 = "INSERT INTO $db"."_tasas_de_produccion (Semana, Cantidad_Ejecutada, Cantidad_Requerida, Cantidad_Teorica_Acumulada) VALUES ";
        ...
        $query2_1 = "INSERT INTO $db"."_tasas_de_produccion (Semana, Cantidad_Ejecutada, Cantidad_Requerida, Cantidad_Teorica_Acumulada) VALUES ";
        ...
        $query3 = "UPDATE $db"."_tasas_de_produccion SET Cantidad_Acumulada=(SELECT SUM(Ejecutado * cantidad_ppto) FROM $db"."_programa_consolidado WHERE Semana=$Semana_primer_registro AND codigo_actividad='$codigo_actividad'), Cantidad_Requerida_Acumulada=(SELECT SUM(Ejecutado * cantidad_ppto) FROM $db"."_programa_consolidado WHERE Semana=$Semana_primer_registro AND codigo_actividad='$codigo_actividad') WHERE Semana=$Semana_primer_registro";
        ...
        while($data3_1=mysqli_fetch_assoc($resultado3_1)){
            $Semana=$data3_1["Semana"];
            $Cantidad_Ejecutada=$data3_1["Cantidad_Ejecutada"];
            $Cantidad_Requerida=$data3_1["Cantidad_Requerida"];
            $Cantidad_Acumulada=$data3_1["Cantidad_Acumulada"];
            $Cantidad_Requerida_Acumulada=$data3_1["Cantidad_Requerida_Acumulada"];
            $arreglo[]=array($Semana,$Cantidad_Ejecutada,$Cantidad_Requerida,$Cantidad_Acumulada,$Cantidad_Requerida_Acumulada);

        }


        $query3_2="";
        $Cantidad_Acumulada = $arreglo[0][3];
        $Cantidad_Requerida_Acumulada = $arreglo[0][4];
        $Semana_primer_registro=$arreglo[0][0];

        for($i=0; $i<count($arreglo); $i++){
            $Cantidad_Acumulada = $Cantidad_Acumulada + ($arreglo[$i][1]);
            $Cantidad_Requerida_Acumulada = $Cantidad_Requerida_Acumulada + ($arreglo[$i][2]);

            $query3_2 .= "UPDATE $db"."_tasas_de_produccion SET Cantidad_Acumulada=$Cantidad_Acumulada, Cantidad_Requerida_Acumulada=$Cantidad_Requerida_Acumulada WHERE Semana=($Semana_primer_registro);";
        ...
        $query_cantidad_requerida="SELECT Semana, Cantidad_Acumulada, Cantidad_Teorica_Acumulada FROM $db"."_tasas_de_produccion WHERE Semana>=$Semana_base";
        $resultado_cantidad_requerida = mysqli_query($conexion, $query_cantidad_requerida);
        $array_cantidad_requerida=array();
        while($data_cantidad_requerida=mysqli_fetch_assoc($resultado_cantidad_requerida)){
            $cantidad_requerida_real=$data_cantidad_requerida["Cantidad_Acumulada"];
            $cantidad_requerida_teorica=$data_cantidad_requerida["Cantidad_Teorica_Acumulada"];
            $semana_cantidad_requerida=$data_cantidad_requerida["Semana"];
            $array_cantidad_requerida["Semana"][]=$semana_cantidad_requerida;
            $array_cantidad_requerida["Cantidad_Requerida_Real"][]=$cantidad_requerida_real;
            $array_cantidad_requerida["Cantidad_Requerida_Teorica"][]=$cantidad_requerida_teorica;
        }


        $semana_inicio_cantidad_requerida=$array_cantidad_requerida["Semana"][0];
        $cantidad_requerida_inicial=$array_cantidad_requerida["Cantidad_Requerida_Real"][0];
        $cantidad_requerida_final=$array_cantidad_requerida["Cantidad_Requerida_Teorica"][0];

        $semana_fin_cantidad_requerida=null;
        for($i=1; $i<count($array_cantidad_requerida["Semana"]); $i++){
            if($array_cantidad_requerida["Cantidad_Requerida_Teorica"][$i]>$cantidad_requerida_final){
              $cantidad_requerida_final=$array_cantidad_requerida["Cantidad_Requerida_Teorica"][$i];
              $semana_fin_cantidad_requerida=$array_cantidad_requerida["Semana"][$i];
            }else{
              $semana_fin_cantidad_requerida=$semana_fin_cantidad_requerida;
            }
        }

        $cantidad_requerida = ($cantidad_requerida_final - $cantidad_requerida_inicial);
        if(!$semana_fin_cantidad_requerida){
          $cantidad_requerida_semanal = $cantidad_requerida;
          $Cantidad_Requerida_Acumulada = $cantidad_requerida_inicial + $cantidad_requerida_semanal;
          $semana_fin_cantidad_requerida = $semana + 1;

          $query3_3 = "UPDATE $db"."_tasas_de_produccion SET Cantidad_Requerida=$cantidad_requerida_semanal WHERE Semana = $semana_fin_cantidad_requerida; ";
          $query3_3 .= "UPDATE $db"."_tasas_de_produccion SET Cantidad_Requerida_Acumulada=$Cantidad_Requerida_Acumulada WHERE Semana >= $semana_fin_cantidad_requerida; ";
        }else{
          $cantidad_requerida_semanal = $cantidad_requerida / ($semana_fin_cantidad_requerida - $semana_inicio_cantidad_requerida);

          $query3_3="";
          $Cantidad_Requerida_Acumulada = $cantidad_requerida_inicial;
          for($i=($semana_inicio_cantidad_requerida+1); $i<($semana_fin_cantidad_requerida+1); $i++){
              $Cantidad_Requerida_Acumulada = $Cantidad_Requerida_Acumulada + $cantidad_requerida_semanal;

              $query3_3 .= "UPDATE $db"."_tasas_de_produccion SET Cantidad_Requerida= $cantidad_requerida_semanal, Cantidad_Requerida_Acumulada=$Cantidad_Requerida_Acumulada WHERE Semana>=($i);";


          }
        }
        $query3_3 .= "UPDATE $db"."_tasas_de_produccion SET Cantidad_Requerida=0 WHERE Semana>$semana_fin_cantidad_requerida;";
        $resultado3_3 = mysqli_multi_query($conexion, $query3_3);
...
function grafico_Seguimiento_Rendimientos($conexion, $semana, $db, $codigo_actividad, $oficiales_teorico, $ayudantes_teorico, $cuadrilla_tipica_teorico, $rendimiento_cuadrilla_tipica_teorico){
    //echo $oficiales_teorico, $cuadrilla_tipica_teorico;
    ...
    if($conteo==0){
        ...
        if(!$resultado){
            ...
            for($i=0; $i< count($arreglo); $i++){
                ...
                $Registro_Semana_consolidado["Rendimiento_Semana"]=round((($Cantidad_Semana / $Dias_Semana) / (ceil($Oficiales_Semana/$Dias_Semana)/$oficiales_teorico)),1);
                //echo "<li> Semana: $Semana_ciclo --> Cantidad: $Cantidad_Semana --> Dias Semana: $Dias_Semana --> Oficiales: $Oficiales_Semana --> Rendimiento_Semana: " . $Registro_Semana_consolidado["Rendimiento_Semana"];
    ...
    if($conteo==0){
        ...
        if(!$resultado){
            ...
            for($i=0; $i< count($arreglo); $i++){
                ...
                $Registro_Semana_consolidado["Oficiales_Promedio_Dia"]= $Oficiales_Promedio_Dia;
                $Registro_Semana_consolidado["Ayudantes_Promedio_Dia"]= $Ayudantes_Promedio_Dia;

                $Registro_Semana_consolidado["cuadrilla_tipica_oficiales"] = round($Registro_Semana_consolidado["Oficiales_Promedio_Dia"] / $oficiales_teorico,1);

                $Registro_Semana_consolidado["cuadrilla_tipica_ayudantes"] = round($Registro_Semana_consolidado["Ayudantes_Promedio_Dia"] / $ayudantes_teorico,1);
    ...
    if($conteo==0){
        ...
        if(!$resultado){
            ...
            while($data=mysqli_fetch_assoc($resultado)){
                ...
                for($i=1; $i<29; $i=$i+4){
                    ...
                    if($Rendimientos[$i-1]=='n' || $Rendimientos[$i-1]=='N' || $Rendimientos[$i-1]==''){
                        ...
                        $Cantidad_Semana = $Cantidad_Semana + $Rendimientos[$i-1];
                        $Oficiales_Semana = $Oficiales_Semana + $Rendimientos[$i];
                        $Ayudantes_Semana = $Ayudantes_Semana + $Rendimientos[$i+1];
            ...
            for($i=0; $i< count($arreglo); $i++){
                ...
                $Registro_Semana_consolidado["Horas_Semana"]=$Horas_Semana;
                $Registro_Semana_consolidado["Dias_Semana"]=$Dias_Semana;
                $Registro_Semana_consolidado["Consumo_Oficiales_Hora"]=($Horas_Semana/($Cantidad_Semana / $Oficiales_Semana))/$Dias_Semana;
                $Registro_Semana_consolidado["Consumo_Ayudantes_Hora"]=($Horas_Semana/($Cantidad_Semana / $Ayudantes_Semana))/$Dias_Semana;
                $Registro_Semana_consolidado["Oficiales_Promedio_Dia"]=($Oficiales_Semana / $Dias_Semana);
                $Registro_Semana_consolidado["Ayudantes_Promedio_Dia"]=($Ayudantes_Semana / $Dias_Semana);
            ...
            for($i=0; $i<count($arreglo_consolidado); $i++){
                ...
                $arreglo_consolidado[$i]["Consumo_Oficiales_Hora_Tendencia"]=$Consumo_Oficiales_Hora_Tendencia;
                ...
                $array['rows'][] = array('c' => array( array('v'=> $Semana),
                                                          array('v'=>round((float)$arreglo_consolidado[$i]["Consumo_Oficiales_Hora"],3), 'f'=>round((float)$arreglo_consolidado[$i]["Consumo_Oficiales_Hora"],3) . " Horas-Oficial/" . "$unidad"),
                                                          array('v'=> round($arreglo_consolidado[$i]["Consumo_Oficiales_Hora"],3)),
                                                          array('v'=>round($arreglo_consolidado[$i]["Consumo_Oficiales_Hora_Tendencia"],3), 'f'=>round($arreglo_consolidado[$i]["Consumo_Oficiales_Hora_Tendencia"],3) . " Horas-Oficial/" . "$unidad"),
                                                          array('v'=>round((1/(($rendimiento_cuadrilla_tipica_teorico/9)/$oficiales_teorico)),3), 'f'=>round((1/(($rendimiento_cuadrilla_tipica_teorico/9)/$oficiales_teorico)),3)   . " Horas-Oficial/" . "$unidad")
                                                         ));
    ...
    if($conteo==0){
        ...
        if(!$resultado){
            ...
            while($data=mysqli_fetch_assoc($resultado)){
                ...
                for($i=1; $i<29; $i=$i+4){
                    ...
                    if($Rendimientos[$i-1]=='n' || $Rendimientos[$i-1]=='N' || $Rendimientos[$i-1]==''){
                        ...
                        $Oficiales_Semana = $Oficiales_Semana + $Rendimientos[$i];
                        $Ayudantes_Semana = $Ayudantes_Semana + $Rendimientos[$i+1];
                        $Horas_Semana = $Horas_Semana + $Rendimientos[$i+2];
                        $Dias_Semana = $Dias_Semana + 1;
                    }
                }
                $Registro_Semana["Semana"]=$Semana_tabla;
            ...
            for($i=0; $i<count($arreglo_consolidado); $i++){
                ...
                $arreglo_consolidado[$i]["Consumo_Ayudantes_Hora_Tendencia"]=$Consumo_Ayudantes_Hora_Tendencia;
                ...
                $array['rows'][] = array('c' => array( array('v'=> $Semana),
                                                          array('v'=>round((float)$arreglo_consolidado[$i]["Consumo_Ayudantes_Hora"],3), 'f'=>round((float)$arreglo_consolidado[$i]["Consumo_Ayudantes_Hora"],3)  . " Horas-Ayudante/" . "$unidad"),
                                                          array('v'=> round($arreglo_consolidado[$i]["Consumo_Ayudantes_Hora"],3)),
                                                          array('v'=> round($arreglo_consolidado[$i]["Consumo_Ayudantes_Hora_Tendencia"],3), 'f'=> round($arreglo_consolidado[$i]["Consumo_Ayudantes_Hora_Tendencia"],3)  . " Horas-Ayudante/" . "$unidad"),
                                                          array('v'=>round((1/(($rendimiento_cuadrilla_tipica_teorico/9)/$ayudantes_teorico)),3), 'f'=>round((1/(($rendimiento_cuadrilla_tipica_teorico/9)/$ayudantes_teorico)),3)   . " Horas-Ayudante/" . "$unidad")
                                                         ));
            }
        }
    }
...
function exportar_cuadrilla_tipica($conexion, $semana, $db, $codigo_actividad, $oficiales_tipica, $ayudantes_tipica, $rendimiento_tipica, $numero_cuadrillas_tipicas){
    ...
    $data=mysqli_fetch_assoc($resultado);
    if($data["COUNT(*)"]==0){
        $query1="INSERT INTO general_cuadrillas_tipicas (proyecto, codigo_actividad, oficiales_tipica, ayudantes_tipica, rendimiento_tipica, numero_cuadrillas_tipicas) VALUES ('$db', '$codigo_actividad', $oficiales_tipica, $ayudantes_tipica, '$rendimiento_tipica', $numero_cuadrillas_tipicas)";
    }else{
