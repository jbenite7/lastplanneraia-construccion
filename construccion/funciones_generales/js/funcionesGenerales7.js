...
var nueva_sem=function(db, carpeta, seccion){
  ...
  $("#btn_guardar_nueva_sem").on("click", function(){
    ...
    opcion="nueva_sem";
    if(carpeta == 1){
      var url= "../funciones_generales/php/nueva_semana.php?db="+db;
    }else{
      var url= "../funciones_generales/php/nueva_semana.php?db="+db;
    }
    $.ajax({
      method:"POST",
      url: "../funciones_generales/php/verificarCICActualizada.php?",
      ...
      if(faltaCalificar != 0){
        ...
        $.ajax({
          ...
          if(semanalConfirmada == 0 && permiso != 'P'){
            window.alert("No se pueden crear la Semana " + (Number(semanaActual)+1) + " hasta que se confirmen los compromisos en la Semana " + semanaActual);
            location.assign("../cambiar_pagina.php?seccion=programacion_semanal&semana="+semanaActual);
          }else{
            if(pdcConteo == 0){
              if(carpeta == 1){
...
var eliminar_sem=function(semana, db, carpeta, seccion){
  ...
  $("#btn_eliminar_sem").on("click", function(){
    ...
    opcion="eliminar_sem";
    if(carpeta == 1){
      var url= "../funciones_generales/php/eliminar_semana.php?db="+db;
    }else{
      var url= "../funciones_generales/php/eliminar_semana.php?db="+db;;
    }
    $.ajax({
      ...
      if(json_info["puedeEliminar"] == "SI"){
        if(carpeta == 1){
          ...
          window.alert("No se puede eliminar una semana menor a la máxima del proyecto (Semana "+json_info["maxSemana"]+")");
          location.reload();
      }
    });
  });
...
var limpiar_buscador = function(tbody, table){
  $('#btn_limpiar_buscador').one("click", function (e) {
    ...
    $.ajax({
      method:"POST",
      url: "../funciones_generales/php/buscadorTabla.php",
      contenttype:"charset=utf-8",
