...
switch($opcion){
    ...
    case 'detalle_compromisos':
        $semana=/*9*/$_POST["semana"];
        $tipo_compromiso=/*"atrasadas_criticas"*/$_POST["tipo_compromiso"];
        $nombre_PAC=/*"general"*/$_POST["nombre_PAC"];
        $clase_PAC=/*"general"*/$_POST["clase_PAC"];
        ...
        if($tipo_compromiso=="criticas"){
            ...
            $script .="AND Critica=1 AND (Activa='0' OR Compromiso IS NULL) AND Atrasada=0";
            ...
            $script .="AND Critica=0 AND (Activa='0' OR Compromiso IS NULL) AND Atrasada=0";
            detalle_compromisos($semana, $conexion, $tipo_compromiso, $script, $db);
        }else if($tipo_compromiso=="atrasadas_criticas"){
            if($clase_PAC=="general"){
                $script="";
            }else if($clase_PAC=="subcontratista"){
                $script="";
            ...
            $script .="AND Atrasada=1 AND Critica=1 AND (Activa='0' OR Compromiso IS NULL) ";
            detalle_compromisos($semana, $conexion, $tipo_compromiso, $script, $db);
        }else if($tipo_compromiso=="atrasadas_no_criticas"){
            if($clase_PAC=="general"){
                $script="";
            }else if($clase_PAC=="subcontratista"){
                $script="";
            ...
            $script .="AND Atrasada=1 AND Critica=0 AND (Activa='0' OR Compromiso IS NULL) ";
            detalle_compromisos($semana, $conexion, $tipo_compromiso, $script, $db);
            ...
            $script .="AND Prog_Sin_Restricciones_100=1 AND Activa='1' AND Compromiso IS NOT NULL ";
    ...
    case 'detalle_restricciones':
        ...
        if($selection==0){
            ...
            $script="AND (Estado_Restricciones<1 AND Estado_Restricciones>0 AND ((D_y_E < 1 AND D_y_E !='N/A') OR (Materiales < 1 AND Materiales !='N/A') OR (MdeO < 1 AND MdeO !='N/A') OR (Equipos < 1 AND Equipos !='N/A') OR (Pdto_Cons < 1 AND Pdto_Cons !='N/A') OR (Modelo < 1 AND Modelo !='N/A'))) ";
            detalle_restricciones($semana, $conexion, $comienzan_en, $script, $db);
        }else if($selection==2){
            $script="AND (Estado_Restricciones<1 AND Estado_Restricciones>0 AND (D_y_E = 1 OR D_y_E ='N/A') AND (Materiales = 1 OR Materiales ='N/A') AND (MdeO = 1 OR MdeO ='N/A') AND (Equipos = 1 OR Equipos ='N/A') AND (Pdto_Cons = 1 OR Pdto_Cons ='N/A') AND (Modelo = 1 OR Modelo ='N/A') AND (Predecesora < 1 AND Predecesora !='N/A')) ";
            detalle_restricciones($semana, $conexion, $comienzan_en, $script, $db);
        }else if($selection==3){
...
function detalle_PAC_General($conexion, $semana, $db){
    $query="SELECT  COUNT(*) FROM $db"."_programacion_semanal WHERE Semana='$semana' AND (Activa='1' OR Activa='NA')  AND PAC IS NOT NULL";
    $resultado= mysqli_query($conexion, $query);
    $data=mysqli_fetch_assoc($resultado);
    $conteo=$data["COUNT(*)"];
    //mysqli_close($conexion);
    if ($conteo==0){
        $arreglo["data"][]=array("Consecutivo" => "","Id" => "","Actividad" => "","Prog_Sin_Restricciones_100" => "","Ubicacion" => "","Sub_Contratista" => "","Responsable_AIA" => "", "Unidad" => "","Compromiso" => "","Ejecutado_Real" => "","P_Completado" => "","PAC" => "","Activa" => "");
        echo json_encode($arreglo);
    } else{
        //require("../conexion.php");
        $query1 = "SELECT * FROM $db"."_programacion_semanal WHERE Semana='$semana' AND (Activa='1' OR Activa='NA') AND PAC IS NOT NULL";
...
function detalle_PAC_Subcontratista($conexion, $semana, $nombre_PAC, $db){
    $query="SELECT  COUNT(*) FROM $db"."_programacion_semanal WHERE Semana='$semana' AND Sub_Contratista='$nombre_PAC' AND (Activa='1' OR Activa='NA')  AND PAC IS NOT NULL";
    $resultado= mysqli_query($conexion, $query);
    $data=mysqli_fetch_assoc($resultado);
    $conteo=$data["COUNT(*)"];
    //mysqli_close($conexion);
    if ($conteo==0){
        ...
        //require("../conexion.php");
        $query1 = "SELECT * FROM $db"."_programacion_semanal WHERE Semana='$semana' AND Sub_Contratista='$nombre_PAC' AND (Activa='1' OR Activa='NA')  AND PAC IS NOT NULL";
        $resultado1 = mysqli_query($conexion, $query1);
        if(!$resultado1){
            die("Error");
        } else{
...
function detalle_PAC_Profesional($conexion, $semana, $nombre_PAC, $db){
    $query="SELECT  COUNT(*) FROM $db"."_programacion_semanal WHERE Semana='$semana' AND Responsable_AIA='$nombre_PAC' AND (Activa='1' OR Activa='NA')  AND PAC IS NOT NULL";
    $resultado= mysqli_query($conexion, $query);
    $data=mysqli_fetch_assoc($resultado);
    $conteo=$data["COUNT(*)"];
    //mysqli_close($conexion);
    if ($conteo==0){
        ...
        //require("../conexion.php");
        $query1 = "SELECT * FROM $db"."_programacion_semanal WHERE Semana='$semana' AND Responsable_AIA='$nombre_PAC' AND (Activa='1' OR Activa='NA')  AND PAC IS NOT NULL";
        $resultado1 = mysqli_query($conexion, $query1);
        if(!$resultado1){
            die("Error");
        } else{
...
function detalle_Pareto_CNC($semana, $conexion, $nombre_PAC, $tipo_CNC, $clase_PAC, $script, $db){
    ...
    //mysqli_close($conexion);
    if ($conteo==0){
        $arreglo["data"][]=array("Semana" => "","Id" => "","Actividad" => "","Ubicacion" => "","Sub_Contratista" => "","Responsable_AIA" => "", "Unidad" => "","Compromiso" => "","Ejecutado_Real" => "","CNC"=>"","Observaciones_CNC"=>"","Activa" => "");
        echo json_encode($arreglo);
    } else{
        //require("../conexion.php");
...
function detalle_compromisos($semana, $conexion, $tipo_compromiso, $script, $db){
    ...
    //mysqli_close($conexion);
    if ($conteo==0){
        $arreglo["data"][]=array("Semana" => "","Id" => "","Actividad" => "", "Prog_Sin_Restricciones_100" => "", "Responsable_AIA" =>"", "Categoria_CNP"=>"","CNP"=>"","Observaciones_CNP" => "");
        echo json_encode($arreglo);
    } else{
        //require("../conexion.php");
...
function detalle_compromisos_sin_rest($semana, $conexion, $tipo_compromiso, $script, $db){
    ...
    //mysqli_close($conexion);
    if ($conteo==0){
        $arreglo["data"][]=array("Semana" => "","Id" => "","Actividad" => "","Estado_Restricciones"=>"","D_y_E"=>"","Materiales" => "","MdeO" => "","Equipos" => "","Predecesora" => "","Pdto_Cons" => "","Modelo" => "","Responsable_AIA" => "","Observaciones" => "");
        echo json_encode($arreglo);
    } else{
        //require("../conexion.php");
...
function detalle_restricciones($semana, $conexion, $comienzan_en, $script, $db){
    $query="SELECT  COUNT(*) FROM $db"."_programa_consolidado WHERE Semana=$semana AND Semanas_Inicio='$comienzan_en' $script";
    $resultado= mysqli_query($conexion, $query);
    $data=mysqli_fetch_assoc($resultado);
    $conteo=$data["COUNT(*)"];
    //mysqli_close($conexion);
    if ($conteo==0){
        $arreglo["data"][]=array("Semana" => "","Id" => "","Actividad" => "","Ruta_Critica" => "","Estado_Restricciones" => "","D_y_E" => "","Materiales" => "","MdeO" => "","Equipos" => "","Predecesora" => "","Pdto_Cons" => "","Modelo" => "","Responsable_AIA" => "","Observaciones" => "");
        echo json_encode($arreglo);
    } else{
        //require("../conexion.php");
        $query1 = "SELECT * FROM $db"."_programa_consolidado WHERE Semana=$semana AND Semanas_Inicio='$comienzan_en' $script";
        $resultado1 = mysqli_query($conexion, $query1);
        if(!$resultado1){
            die("Error");
        } else{
