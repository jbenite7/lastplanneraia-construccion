<?php
    ...
    $db=/*"cedi_pasto"*/$_GET['db'];
    $opcion=/*"PAC"*/$_POST["opcion"];
    $semana=/*10*/$_GET["semana"];
...
function grafico_PAC_general($conexion, $semana, $db, $nombre, $script_ultimas_semanas){
    ...
    if($conteo>0){
        ...
        if(!$resultado){
            ...
            $array['cols'][3] = array('id' => 'PAC_Acum' , 'label' => 'PAC Tendencia' , 'type' => 'number');
            ...
            $array['cols'][6] = array('id' => 'P_Completado_Acum' , 'label' => '% Completado Tendencia' , 'type' => 'number');
        } else{
            $array['cols'][0] = array('id' => 'Semanas' , 'label' => 'Semanas' , 'type' => 'string');
            $array['cols'][1] = array('id' => 'PAC' , 'label' => 'PAC' , 'type' => 'number');
            $array['cols'][2] = array('id' => 'PAC_label' , 'label' => 'PAC' , 'type' => 'string', 'role' => 'annotation');
    ...
    while($semana<=$semana1){
        ...
        if ($conteo==0){
            ...
            $query1= "SELECT 'consolidado general' AS 'subcontratista_profesional', 
                             ...
                             ((SELECT COUNT(*) FROM $db"."_programacion_semanal WHERE Critica=1 AND Semana=$semana AND Activa=1 AND Atrasada=0 AND Compromiso>0 AND Compromiso IS NOT NULL) / (SELECT COUNT(*) FROM $db"."_programacion_semanal WHERE Critica=1 AND Semana=$semana AND Atrasada=0)) ,3) END) AS 'Criticas_Comp',
                             ...
                             ((SELECT COUNT(*) FROM $db"."_programacion_semanal WHERE Critica=0 AND Semana=$semana AND (Activa=1 OR Activa='NA') AND Atrasada=0 AND Compromiso IS NOT NULL) / (SELECT COUNT(*) FROM $db"."_programacion_semanal WHERE Critica=0 AND Semana=$semana AND Atrasada=0)) ,3) END) AS 'No_Criticas_Comp',
                             ...
                             ((SELECT COUNT(*) FROM $db"."_programacion_semanal WHERE Critica=1 AND Semana=$semana AND Activa=1 AND Atrasada=1 AND Compromiso IS NOT NULL) / (SELECT COUNT(*) FROM $db"."_programacion_semanal WHERE Critica=1 AND Semana=$semana AND Atrasada=1)) ,3) END) AS 'Atrasadas_Criticas_Comp',
                             ...
                             ((SELECT COUNT(*) FROM $db"."_programacion_semanal WHERE Critica=0 AND Semana=$semana AND Activa=1 AND Atrasada=1 AND Compromiso IS NOT NULL) / (SELECT COUNT(*) FROM $db"."_programacion_semanal WHERE Critica=0 AND Semana=$semana AND Atrasada=1)) ,3) END) AS 'Atrasadas_No_Criticas_Comp',
                             ...
                             ((SELECT COUNT(*) FROM $db"."_programacion_semanal WHERE Semana=$semana AND Activa=1 AND Prog_Sin_Restricciones_100=1 AND Compromiso IS NOT NULL) / (SELECT COUNT(*) FROM $db"."_programacion_semanal WHERE Semana=$semana AND Activa=1)) ,3) END) AS 'Comp_Sin_Rest_100',

                             (SELECT COUNT(*) FROM $db"."_programa_consolidado WHERE Semana=$semana AND Semanas_Inicio=1) AS 'Act_Inician_Sem_1',

                             (SELECT CASE WHEN (SELECT COUNT(*) FROM $db"."_programa_consolidado WHERE Semana=$semana AND Semanas_Inicio=1)=0 THEN 0 ELSE ROUND( (SELECT COUNT(*) FROM $db"."_programa_consolidado WHERE Semana=$semana AND Semanas_Inicio=1 AND Estado_Restricciones=0) / (SELECT COUNT(*) FROM $db"."_programa_consolidado WHERE Semana=$semana AND Semanas_Inicio=1) ,3) END) AS 'Act_0_Lib_Sem_1',

                             (SELECT CASE WHEN (SELECT COUNT(*) FROM $db"."_programa_consolidado WHERE Semana=$semana AND Semanas_Inicio=1)=0 THEN 0 ELSE ROUND( (SELECT COUNT(*) FROM $db"."_programa_consolidado WHERE Semana=$semana AND Semanas_Inicio=1 AND Estado_Restricciones<1 AND Estado_Restricciones>0) / (SELECT COUNT(*) FROM $db"."_programa_consolidado WHERE Semana=$semana AND Semanas_Inicio=1) ,3) END) AS 'Act_Par_Lib_Sem_1',

                             (SELECT CASE WHEN (SELECT COUNT(*) FROM $db"."_programa_consolidado WHERE Semana=$semana AND Semanas_Inicio=1)=0 THEN 0 ELSE ROUND( (SELECT COUNT(*) FROM $db"."_programa_consolidado WHERE Semana=$semana AND Semanas_Inicio=1 AND Estado_Restricciones=1) / (SELECT COUNT(*) FROM $db"."_programa_consolidado WHERE Semana=$semana AND Semanas_Inicio=1) ,3) END) AS 'Act_100_Lib_Sem_1',

                             (SELECT COUNT(*) FROM $db"."_programa_consolidado WHERE Semana=$semana AND Semanas_Inicio=2) AS Act_Inician_Sem_2,

                             (SELECT CASE WHEN (SELECT COUNT(*) FROM $db"."_programa_consolidado WHERE Semana=$semana AND Semanas_Inicio=2)=0 THEN 0 ELSE ROUND( (SELECT COUNT(*) FROM $db"."_programa_consolidado WHERE Semana=$semana AND Semanas_Inicio=2 AND Estado_Restricciones=0) / (SELECT COUNT(*) FROM $db"."_programa_consolidado WHERE Semana=$semana AND Semanas_Inicio=2) ,3) END) AS Act_0_Lib_Sem_2,

                             (SELECT CASE WHEN (SELECT COUNT(*) FROM $db"."_programa_consolidado WHERE Semana=$semana AND Semanas_Inicio=2)=0 THEN 0 ELSE ROUND( (SELECT COUNT(*) FROM $db"."_programa_consolidado WHERE Semana=$semana AND Semanas_Inicio=2 AND Estado_Restricciones<1 AND Estado_Restricciones>0) / (SELECT COUNT(*) FROM $db"."_programa_consolidado WHERE Semana=$semana AND Semanas_Inicio=2) ,3) END) AS Act_Par_Lib_Sem_2,

                             (SELECT CASE WHEN (SELECT COUNT(*) FROM $db"."_programa_consolidado WHERE Semana=$semana AND Semanas_Inicio=2)=0 THEN 0 ELSE ROUND( (SELECT COUNT(*) FROM $db"."_programa_consolidado WHERE Semana=$semana AND Semanas_Inicio=2 AND Estado_Restricciones=1) / (SELECT COUNT(*) FROM $db"."_programa_consolidado WHERE Semana=$semana AND Semanas_Inicio=2) ,3) END) AS Act_100_Lib_Sem_2,

                             (SELECT COUNT(*) FROM $db"."_programa_consolidado WHERE Semana=$semana AND Semanas_Inicio=3) AS Act_Inician_Sem_3,

                             (SELECT CASE WHEN (SELECT COUNT(*) FROM $db"."_programa_consolidado WHERE Semana=$semana AND Semanas_Inicio=3)=0 THEN 0 ELSE ROUND( (SELECT COUNT(*) FROM $db"."_programa_consolidado WHERE Semana=$semana AND Semanas_Inicio=3 AND Estado_Restricciones=0) / (SELECT COUNT(*) FROM $db"."_programa_consolidado WHERE Semana=$semana AND Semanas_Inicio=3) ,3) END) AS Act_0_Lib_Sem_3,

                             (SELECT CASE WHEN (SELECT COUNT(*) FROM $db"."_programa_consolidado WHERE Semana=$semana AND Semanas_Inicio=3)=0 THEN 0 ELSE ROUND( (SELECT COUNT(*) FROM $db"."_programa_consolidado WHERE Semana=$semana AND Semanas_Inicio=3 AND Estado_Restricciones<1 AND Estado_Restricciones>0) / (SELECT COUNT(*) FROM $db"."_programa_consolidado WHERE Semana=$semana AND Semanas_Inicio=3) ,3) END) AS Act_Par_Lib_Sem_3,

                             (SELECT CASE WHEN (SELECT COUNT(*) FROM $db"."_programa_consolidado WHERE Semana=$semana AND Semanas_Inicio=3)=0 THEN 0 ELSE ROUND( (SELECT COUNT(*) FROM $db"."_programa_consolidado WHERE Semana=$semana AND Semanas_Inicio=3 AND Estado_Restricciones=1) / (SELECT COUNT(*) FROM $db"."_programa_consolidado WHERE Semana=$semana AND Semanas_Inicio=3) ,3) END) AS Act_100_Lib_Sem_3,

                             (SELECT COUNT(*) FROM $db"."_programa_consolidado WHERE Semana=$semana AND Semanas_Inicio=4) AS Act_Inician_Sem_4,

                             (SELECT CASE WHEN (SELECT COUNT(*) FROM $db"."_programa_consolidado WHERE Semana=$semana AND Semanas_Inicio=4)=0 THEN 0 ELSE ROUND( (SELECT COUNT(*) FROM $db"."_programa_consolidado WHERE Semana=$semana AND Semanas_Inicio=4 AND Estado_Restricciones=0) / (SELECT COUNT(*) FROM $db"."_programa_consolidado WHERE Semana=$semana AND Semanas_Inicio=4) ,3) END) AS Act_0_Lib_Sem_4,

                             (SELECT CASE WHEN (SELECT COUNT(*) FROM $db"."_programa_consolidado WHERE Semana=$semana AND Semanas_Inicio=4)=0 THEN 0 ELSE ROUND( (SELECT COUNT(*) FROM $db"."_programa_consolidado WHERE Semana=$semana AND Semanas_Inicio=4 AND Estado_Restricciones<1 AND Estado_Restricciones>0) / (SELECT COUNT(*) FROM $db"."_programa_consolidado WHERE Semana=$semana AND Semanas_Inicio=4) ,3) END) AS Act_Par_Lib_Sem_4,

                             (SELECT CASE WHEN (SELECT COUNT(*) FROM $db"."_programa_consolidado WHERE Semana=$semana AND Semanas_Inicio=4)=0 THEN 0 ELSE ROUND( (SELECT COUNT(*) FROM $db"."_programa_consolidado WHERE Semana=$semana AND Semanas_Inicio=4 AND Estado_Restricciones=1) / (SELECT COUNT(*) FROM $db"."_programa_consolidado WHERE Semana=$semana AND Semanas_Inicio=4) ,3) END) AS Act_100_Lib_Sem_4,

                             (SELECT COUNT(*) FROM $db"."_programa_consolidado WHERE Semana=$semana AND Semanas_Inicio=5) AS Act_Inician_Sem_5,

                             (SELECT CASE WHEN (SELECT COUNT(*) FROM $db"."_programa_consolidado WHERE Semana=$semana AND Semanas_Inicio=5)=0 THEN 0 ELSE ROUND( (SELECT COUNT(*) FROM $db"."_programa_consolidado WHERE Semana=$semana AND Semanas_Inicio=5 AND Estado_Restricciones=0) / (SELECT COUNT(*) FROM $db"."_programa_consolidado WHERE Semana=$semana AND Semanas_Inicio=5) ,3) END) AS Act_0_Lib_Sem_5,

                             (SELECT CASE WHEN (SELECT COUNT(*) FROM $db"."_programa_consolidado WHERE Semana=$semana AND Semanas_Inicio=5)=0 THEN 0 ELSE ROUND( (SELECT COUNT(*) FROM $db"."_programa_consolidado WHERE Semana=$semana AND Semanas_Inicio=5 AND Estado_Restricciones<1 AND Estado_Restricciones>0) / (SELECT COUNT(*) FROM $db"."_programa_consolidado WHERE Semana=$semana AND Semanas_Inicio=5) ,3) END) AS Act_Par_Lib_Sem_5,

                             (SELECT CASE WHEN (SELECT COUNT(*) FROM $db"."_programa_consolidado WHERE Semana=$semana AND Semanas_Inicio=5)=0 THEN 0 ELSE ROUND( (SELECT COUNT(*) FROM $db"."_programa_consolidado WHERE Semana=$semana AND Semanas_Inicio=5 AND Estado_Restricciones=1) / (SELECT COUNT(*) FROM $db"."_programa_consolidado WHERE Semana=$semana AND Semanas_Inicio=5) ,3) END) AS Act_100_Lib_Sem_5,

                             (SELECT COUNT(*) FROM $db"."_programa_consolidado WHERE Semana=$semana AND Semanas_Inicio=6) AS Act_Inician_Sem_6,

                             (SELECT CASE WHEN (SELECT COUNT(*) FROM $db"."_programa_consolidado WHERE Semana=$semana AND Semanas_Inicio=6)=0 THEN 0 ELSE ROUND( (SELECT COUNT(*) FROM $db"."_programa_consolidado WHERE Semana=$semana AND Semanas_Inicio=6 AND Estado_Restricciones=0) / (SELECT COUNT(*) FROM $db"."_programa_consolidado WHERE Semana=$semana AND Semanas_Inicio=6) ,3) END) AS Act_0_Lib_Sem_6,

                             (SELECT CASE WHEN (SELECT COUNT(*) FROM $db"."_programa_consolidado WHERE Semana=$semana AND Semanas_Inicio=6)=0 THEN 0 ELSE ROUND( (SELECT COUNT(*) FROM $db"."_programa_consolidado WHERE Semana=$semana AND Semanas_Inicio=6 AND Estado_Restricciones<1 AND Estado_Restricciones>0) / (SELECT COUNT(*) FROM $db"."_programa_consolidado WHERE Semana=$semana AND Semanas_Inicio=6) ,3) END) AS Act_Par_Lib_Sem_6,

                             (SELECT CASE WHEN (SELECT COUNT(*) FROM $db"."_programa_consolidado WHERE Semana=$semana AND Semanas_Inicio=6)=0 THEN 0 ELSE ROUND( (SELECT COUNT(*) FROM $db"."_programa_consolidado WHERE Semana=$semana AND Semanas_Inicio=6 AND Estado_Restricciones=1) / (SELECT COUNT(*) FROM $db"."_programa_consolidado WHERE Semana=$semana AND Semanas_Inicio=6) ,3) END) AS Act_100_Lib_Sem_6
                              ;";
            ...
            $query1= "SELECT 'consolidado general' AS 'subcontratista_profesional', 
                             ...
                             (SELECT CASE WHEN (SELECT COUNT(*) FROM $db"."_programacion_semanal WHERE Critica=1 AND Semana=$semana AND Atrasada=0)=0 THEN 'NA' ELSE ROUND( 
                             ((SELECT COUNT(*) FROM $db"."_programacion_semanal WHERE Critica=1 AND Semana=$semana AND Activa=1 AND Atrasada=0 AND Compromiso IS NOT NULL) / (SELECT COUNT(*) FROM $db"."_programacion_semanal WHERE Critica=1 AND Semana=$semana AND Atrasada=0)) ,3) END) AS 'Criticas_Comp',

                             (SELECT CASE WHEN (SELECT COUNT(*) FROM $db"."_programacion_semanal WHERE Critica=0 AND Semana=$semana AND Atrasada=0)=0 THEN 'NA' ELSE ROUND( 
                             ((SELECT COUNT(*) FROM $db"."_programacion_semanal WHERE Critica=0 AND Semana=$semana AND (Activa=1 OR Activa='NA') AND Atrasada=0 AND Atrasada=0 AND Compromiso IS NOT NULL) / (SELECT COUNT(*) FROM $db"."_programacion_semanal WHERE Critica=0 AND Semana=$semana AND Atrasada=0)) ,3) END) AS 'No_Criticas_Comp',
        ...
        while($row=mysqli_fetch_assoc($resultado2)){
            ...
            $query4_3 = "UPDATE $db"."_indicadores_generales SET 
                                                               ...
                                                               Atrasadas_Comp_Acum='$Atrasadas_Criticas_Comp_Acum',
        ...
        while($row2=mysqli_fetch_assoc($resultado5)){
            ...
            if ($conteo2==0){
                ...
                $query7= "SELECT '$nombre' AS 'subcontratista_profesional', 
                             ...
                             ((SELECT COUNT(*) FROM $db"."_programacion_semanal WHERE Critica=1 AND Semana=$semana AND Activa=1 AND Atrasada=0 AND Responsable_AIA='$nombre' AND Compromiso IS NOT NULL) / (SELECT COUNT(*) FROM $db"."_programacion_semanal WHERE Critica=1 AND Semana=$semana AND Atrasada=0 AND Responsable_AIA='$nombre')) ,3) END) AS 'Criticas_Comp',
                             ...
                             ((SELECT COUNT(*) FROM $db"."_programacion_semanal WHERE Critica=0 AND Semana=$semana AND (Activa=1 OR Activa='NA') AND Atrasada=0 AND Responsable_AIA='$nombre' AND Compromiso IS NOT NULL) / (SELECT COUNT(*) FROM $db"."_programacion_semanal WHERE Critica=0 AND Semana=$semana AND Atrasada=0 AND Responsable_AIA='$nombre')) ,3) END) AS 'No_Criticas_Comp',
                             ...
                             ((SELECT COUNT(*) FROM $db"."_programacion_semanal WHERE Critica=1 AND Semana=$semana AND Activa=1 AND Atrasada=1 AND Responsable_AIA='$nombre' AND Compromiso IS NOT NULL) / (SELECT COUNT(*) FROM $db"."_programacion_semanal WHERE Critica=1 AND Semana=$semana AND Atrasada=1 AND Responsable_AIA='$nombre')) ,3) END) AS 'Atrasadas_Criticas_Comp',
                             ...
                             ((SELECT COUNT(*) FROM $db"."_programacion_semanal WHERE Semana=$semana AND Activa=1 AND Prog_Sin_Restricciones_100=1 AND Responsable_AIA='$nombre' AND Compromiso IS NOT NULL) / (SELECT COUNT(*) FROM $db"."_programacion_semanal WHERE Semana=$semana AND Activa=1 AND Responsable_AIA='$nombre')) ,3) END) AS 'Comp_Sin_Rest_100',

                             (SELECT COUNT(*) FROM $db"."_programa_consolidado WHERE Semana=$semana AND Semanas_Inicio=1 AND Responsable_AIA='$nombre') AS 'Act_Inician_Sem_1',

                             (SELECT CASE WHEN (SELECT COUNT(*) FROM $db"."_programa_consolidado WHERE Semana=$semana AND Semanas_Inicio=1 AND Responsable_AIA='$nombre')=0 THEN 0 ELSE ROUND( (SELECT COUNT(*) FROM $db"."_programa_consolidado WHERE Semana=$semana AND Semanas_Inicio=1 AND Estado_Restricciones=0 AND Responsable_AIA='$nombre') / (SELECT COUNT(*) FROM $db"."_programa_consolidado WHERE Semana=$semana AND Semanas_Inicio=1 AND Responsable_AIA='$nombre') ,3) END) AS 'Act_0_Lib_Sem_1',

                             (SELECT CASE WHEN (SELECT COUNT(*) FROM $db"."_programa_consolidado WHERE Semana=$semana AND Semanas_Inicio=1 AND Responsable_AIA='$nombre')=0 THEN 0 ELSE ROUND( (SELECT COUNT(*) FROM $db"."_programa_consolidado WHERE Semana=$semana AND Semanas_Inicio=1 AND Estado_Restricciones<1 AND Estado_Restricciones>0 AND Responsable_AIA='$nombre') / (SELECT COUNT(*) FROM $db"."_programa_consolidado WHERE Semana=$semana AND Semanas_Inicio=1 AND Responsable_AIA='$nombre') ,3) END) AS 'Act_Par_Lib_Sem_1',

                             (SELECT CASE WHEN (SELECT COUNT(*) FROM $db"."_programa_consolidado WHERE Semana=$semana AND Semanas_Inicio=1 AND Responsable_AIA='$nombre')=0 THEN 0 ELSE ROUND( (SELECT COUNT(*) FROM $db"."_programa_consolidado WHERE Semana=$semana AND Semanas_Inicio=1 AND Estado_Restricciones=1 AND Responsable_AIA='$nombre') / (SELECT COUNT(*) FROM $db"."_programa_consolidado WHERE Semana=$semana AND Semanas_Inicio=1 AND Responsable_AIA='$nombre') ,3) END) AS 'Act_100_Lib_Sem_1',

                             (SELECT COUNT(*) FROM $db"."_programa_consolidado WHERE Semana=$semana AND Semanas_Inicio=2 AND Responsable_AIA='$nombre') AS Act_Inician_Sem_2,

                             (SELECT CASE WHEN (SELECT COUNT(*) FROM $db"."_programa_consolidado WHERE Semana=$semana AND Semanas_Inicio=2 AND Responsable_AIA='$nombre')=0 THEN 0 ELSE ROUND( (SELECT COUNT(*) FROM $db"."_programa_consolidado WHERE Semana=$semana AND Semanas_Inicio=2 AND Estado_Restricciones=0 AND Responsable_AIA='$nombre') / (SELECT COUNT(*) FROM $db"."_programa_consolidado WHERE Semana=$semana AND Semanas_Inicio=2 AND Responsable_AIA='$nombre') ,3) END) AS Act_0_Lib_Sem_2,

                             (SELECT CASE WHEN (SELECT COUNT(*) FROM $db"."_programa_consolidado WHERE Semana=$semana AND Semanas_Inicio=2 AND Responsable_AIA='$nombre')=0 THEN 0 ELSE ROUND( (SELECT COUNT(*) FROM $db"."_programa_consolidado WHERE Semana=$semana AND Semanas_Inicio=2 AND Estado_Restricciones<1 AND Estado_Restricciones>0 AND Responsable_AIA='$nombre') / (SELECT COUNT(*) FROM $db"."_programa_consolidado WHERE Semana=$semana AND Semanas_Inicio=2 AND Responsable_AIA='$nombre') ,3) END) AS Act_Par_Lib_Sem_2,

                             (SELECT CASE WHEN (SELECT COUNT(*) FROM $db"."_programa_consolidado WHERE Semana=$semana AND Semanas_Inicio=2 AND Responsable_AIA='$nombre')=0 THEN 0 ELSE ROUND( (SELECT COUNT(*) FROM $db"."_programa_consolidado WHERE Semana=$semana AND Semanas_Inicio=2 AND Estado_Restricciones=1 AND Responsable_AIA='$nombre') / (SELECT COUNT(*) FROM $db"."_programa_consolidado WHERE Semana=$semana AND Semanas_Inicio=2 AND Responsable_AIA='$nombre') ,3) END) AS Act_100_Lib_Sem_2,

                             (SELECT COUNT(*) FROM $db"."_programa_consolidado WHERE Semana=$semana AND Semanas_Inicio=3 AND Responsable_AIA='$nombre') AS Act_Inician_Sem_3,

                             (SELECT CASE WHEN (SELECT COUNT(*) FROM $db"."_programa_consolidado WHERE Semana=$semana AND Semanas_Inicio=3 AND Responsable_AIA='$nombre')=0 THEN 0 ELSE ROUND( (SELECT COUNT(*) FROM $db"."_programa_consolidado WHERE Semana=$semana AND Semanas_Inicio=3 AND Estado_Restricciones=0 AND Responsable_AIA='$nombre') / (SELECT COUNT(*) FROM $db"."_programa_consolidado WHERE Semana=$semana AND Semanas_Inicio=3 AND Responsable_AIA='$nombre') ,3) END) AS Act_0_Lib_Sem_3,

                             (SELECT CASE WHEN (SELECT COUNT(*) FROM $db"."_programa_consolidado WHERE Semana=$semana AND Semanas_Inicio=3 AND Responsable_AIA='$nombre')=0 THEN 0 ELSE ROUND( (SELECT COUNT(*) FROM $db"."_programa_consolidado WHERE Semana=$semana AND Semanas_Inicio=3 AND Estado_Restricciones<1 AND Estado_Restricciones>0 AND Responsable_AIA='$nombre') / (SELECT COUNT(*) FROM $db"."_programa_consolidado WHERE Semana=$semana AND Semanas_Inicio=3 AND Responsable_AIA='$nombre') ,3) END) AS Act_Par_Lib_Sem_3,

                             (SELECT CASE WHEN (SELECT COUNT(*) FROM $db"."_programa_consolidado WHERE Semana=$semana AND Semanas_Inicio=3 AND Responsable_AIA='$nombre')=0 THEN 0 ELSE ROUND( (SELECT COUNT(*) FROM $db"."_programa_consolidado WHERE Semana=$semana AND Semanas_Inicio=3 AND Estado_Restricciones=1 AND Responsable_AIA='$nombre') / (SELECT COUNT(*) FROM $db"."_programa_consolidado WHERE Semana=$semana AND Semanas_Inicio=3 AND Responsable_AIA='$nombre') ,3) END) AS Act_100_Lib_Sem_3,

                             (SELECT COUNT(*) FROM $db"."_programa_consolidado WHERE Semana=$semana AND Semanas_Inicio=4 AND Responsable_AIA='$nombre') AS Act_Inician_Sem_4,

                             (SELECT CASE WHEN (SELECT COUNT(*) FROM $db"."_programa_consolidado WHERE Semana=$semana AND Semanas_Inicio=4 AND Responsable_AIA='$nombre')=0 THEN 0 ELSE ROUND( (SELECT COUNT(*) FROM $db"."_programa_consolidado WHERE Semana=$semana AND Semanas_Inicio=4 AND Estado_Restricciones=0 AND Responsable_AIA='$nombre') / (SELECT COUNT(*) FROM $db"."_programa_consolidado WHERE Semana=$semana AND Semanas_Inicio=4 AND Responsable_AIA='$nombre') ,3) END) AS Act_0_Lib_Sem_4,

                             (SELECT CASE WHEN (SELECT COUNT(*) FROM $db"."_programa_consolidado WHERE Semana=$semana AND Semanas_Inicio=4 AND Responsable_AIA='$nombre')=0 THEN 0 ELSE ROUND( (SELECT COUNT(*) FROM $db"."_programa_consolidado WHERE Semana=$semana AND Semanas_Inicio=4 AND Estado_Restricciones<1 AND Estado_Restricciones>0 AND Responsable_AIA='$nombre') / (SELECT COUNT(*) FROM $db"."_programa_consolidado WHERE Semana=$semana AND Semanas_Inicio=4 AND Responsable_AIA='$nombre') ,3) END) AS Act_Par_Lib_Sem_4,

                             (SELECT CASE WHEN (SELECT COUNT(*) FROM $db"."_programa_consolidado WHERE Semana=$semana AND Semanas_Inicio=4 AND Responsable_AIA='$nombre')=0 THEN 0 ELSE ROUND( (SELECT COUNT(*) FROM $db"."_programa_consolidado WHERE Semana=$semana AND Semanas_Inicio=4 AND Estado_Restricciones=1 AND Responsable_AIA='$nombre') / (SELECT COUNT(*) FROM $db"."_programa_consolidado WHERE Semana=$semana AND Semanas_Inicio=4 AND Responsable_AIA='$nombre') ,3) END) AS Act_100_Lib_Sem_4,

                             (SELECT COUNT(*) FROM $db"."_programa_consolidado WHERE Semana=$semana AND Semanas_Inicio=5) AS Act_Inician_Sem_5,

                             (SELECT CASE WHEN (SELECT COUNT(*) FROM $db"."_programa_consolidado WHERE Semana=$semana AND Semanas_Inicio=5 AND Responsable_AIA='$nombre')=0 THEN 0 ELSE ROUND( (SELECT COUNT(*) FROM $db"."_programa_consolidado WHERE Semana=$semana AND Semanas_Inicio=5 AND Estado_Restricciones=0 AND Responsable_AIA='$nombre') / (SELECT COUNT(*) FROM $db"."_programa_consolidado WHERE Semana=$semana AND Semanas_Inicio=5 AND Responsable_AIA='$nombre') ,3) END) AS Act_0_Lib_Sem_5,

                             (SELECT CASE WHEN (SELECT COUNT(*) FROM $db"."_programa_consolidado WHERE Semana=$semana AND Semanas_Inicio=5 AND Responsable_AIA='$nombre')=0 THEN 0 ELSE ROUND( (SELECT COUNT(*) FROM $db"."_programa_consolidado WHERE Semana=$semana AND Semanas_Inicio=5 AND Estado_Restricciones<1 AND Estado_Restricciones>0 AND Responsable_AIA='$nombre') / (SELECT COUNT(*) FROM $db"."_programa_consolidado WHERE Semana=$semana AND Semanas_Inicio=5 AND Responsable_AIA='$nombre') ,3) END) AS Act_Par_Lib_Sem_5,

                             (SELECT CASE WHEN (SELECT COUNT(*) FROM $db"."_programa_consolidado WHERE Semana=$semana AND Semanas_Inicio=5 AND Responsable_AIA='$nombre')=0 THEN 0 ELSE ROUND( (SELECT COUNT(*) FROM $db"."_programa_consolidado WHERE Semana=$semana AND Semanas_Inicio=5 AND Estado_Restricciones=1 AND Responsable_AIA='$nombre') / (SELECT COUNT(*) FROM $db"."_programa_consolidado WHERE Semana=$semana AND Semanas_Inicio=5 AND Responsable_AIA='$nombre') ,3) END) AS Act_100_Lib_Sem_5,

                             (SELECT COUNT(*) FROM $db"."_programa_consolidado WHERE Semana=$semana AND Semanas_Inicio=6 AND Responsable_AIA='$nombre') AS Act_Inician_Sem_6,

                             (SELECT CASE WHEN (SELECT COUNT(*) FROM $db"."_programa_consolidado WHERE Semana=$semana AND Semanas_Inicio=6 AND Responsable_AIA='$nombre')=0 THEN 0 ELSE ROUND( (SELECT COUNT(*) FROM $db"."_programa_consolidado WHERE Semana=$semana AND Semanas_Inicio=6 AND Estado_Restricciones=0 AND Responsable_AIA='$nombre') / (SELECT COUNT(*) FROM $db"."_programa_consolidado WHERE Semana=$semana AND Semanas_Inicio=6 AND Responsable_AIA='$nombre') ,3) END) AS Act_0_Lib_Sem_6,

                             (SELECT CASE WHEN (SELECT COUNT(*) FROM $db"."_programa_consolidado WHERE Semana=$semana AND Semanas_Inicio=6 AND Responsable_AIA='$nombre')=0 THEN 0 ELSE ROUND( (SELECT COUNT(*) FROM $db"."_programa_consolidado WHERE Semana=$semana AND Semanas_Inicio=6 AND Estado_Restricciones<1 AND Estado_Restricciones>0 AND Responsable_AIA='$nombre') / (SELECT COUNT(*) FROM $db"."_programa_consolidado WHERE Semana=$semana AND Semanas_Inicio=6 AND Responsable_AIA='$nombre') ,3) END) AS Act_Par_Lib_Sem_6,

                             (SELECT CASE WHEN (SELECT COUNT(*) FROM $db"."_programa_consolidado WHERE Semana=$semana AND Semanas_Inicio=6 AND Responsable_AIA='$nombre')=0 THEN 0 ELSE ROUND( (SELECT COUNT(*) FROM $db"."_programa_consolidado WHERE Semana=$semana AND Semanas_Inicio=6 AND Estado_Restricciones=1 AND Responsable_AIA='$nombre') / (SELECT COUNT(*) FROM $db"."_programa_consolidado WHERE Semana=$semana AND Semanas_Inicio=6 AND Responsable_AIA='$nombre') ,3) END) AS Act_100_Lib_Sem_6
                              ;";
            ...
            $query7_0= "SELECT (SELECT CASE WHEN (SELECT COUNT(*) FROM $db"."_programacion_semanal WHERE Critica=1 AND Semana=$semana AND Atrasada=0 AND Responsable_AIA='$nombre')=0 THEN 'NA' ELSE ROUND( 
                             ...
                             (SELECT COUNT(*) FROM $db"."_programa_consolidado WHERE Semana=$semana AND Semanas_Inicio=5 AND Responsable_AIA='$nombre') AS Act_Inician_Sem_5,
