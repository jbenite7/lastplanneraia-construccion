...
| `phpstan-baseline.json` | baseline | Cualquier fingerprint de PHPStan nuevo; hoy tolera **cero** (`"fingerprints": []`, verificado el 2026-08-10) — `scripts/design-system-phpstan-baseline.mjs:75-92`. Las cinco legacy que tenía (de `ListadoActividadesApiController` y `SemiAutoService`) se retiraron en `9011c99c` al borrarse esos archivos con el PDC v1 |
| `a11y-baseline.json` | baseline | Hoy exige `fingerprints` **vacío**: tolerancia cero, todo pasa por excepción — `tests/design-system/accessibility.test.mjs:258` (verificado el 2026-08-07: el archivo sigue con `"fingerprints": []`) |
