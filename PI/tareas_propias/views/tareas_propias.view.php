            ...
            <nav class="navbar navbar-expand-m navbar-dark ">
              <a class="navbar-brand" href="#">
                <ul>
                    ...
                    <li class="lps">LPS</li>
                    <li class="pagina"> - Tareas Propias / Semana <?php echo $semana ?></li>
                </ul>
              </a>
                                    ...
                                    if(($Max_Semana-2)>=$i){
                                        ...
                                        <li class='nav-item dropdown active' style='padding: 2px 16px'>
                                            ...
                                            <div class='dropdown-menu show' aria-labelledby='navbarDropdown'>
                                                ...
                                                <a class='dropdown-item active' href='../cambiar_pagina.php?seccion=tareas_propias&semana=$i'>Tareas Propias</a>
	...
	<script>
		...
		var guardar = function(){
			$("#btn_guardar").on("click", function(e){
                ...
                frm=frm+"&"+Ejecutado+"&"+Relevancia+"&"+Observaciones;
                console.log(frm);
				$.ajax({
					method: "POST",
					url: "../tareas_propias/guardar_tareas_propias.php<?php echo "?db=$db&semana=$semana"?>",
					...
					var posicion=$('.dataTables_scrollBody').scrollTop();
                    //location.assign("../scroll_general.php?scroll="+posicion+"&seccion=propias")
                    //var posicion=$('.dataTables_scrollBody').scrollTop();
					//listar();
                    location.assign("../scroll_general.php?scroll="+posicion+"&seccion=tareas_propias")
                    //cambiar_posicion(posicion);
				});
			});
		}
        ...
        var eliminar = function(){
			$("#eliminar-usuario").on("click", function(){
				...
				$.ajax({
					method:"POST",
					url: "../tareas_propias/guardar_tareas_propias.php?db=<?php echo $db?>",
                    contenttype:"charset=utf-8",
        ...
        var listar=function(clase){
            ...
            if(isset($_SESSION['lookahead_tpr'])){
                if($_SESSION['lookahead_tpr']==1){
                    echo 1;
                }else{
                    echo 0;
                }
            ...
            if(isset($_SESSION['no_iniciadas_tpr'])){
                if($_SESSION['no_iniciadas_tpr']==1){
                    echo 1;
                }else{
                    echo 0;
                }
            ...
            if(isset($_SESSION['en_ejecucion_tpr'])){
                if($_SESSION['en_ejecucion_tpr']==1){
                    echo 1;
                }else{
                    echo 0;
                }
            ...
            if(isset($_SESSION['terminadas_tpr'])){
                if($_SESSION['terminadas_tpr']==1){
                    echo 1;
                }else{
                    echo 0;
                }
            ...
            <?php 
                require("../conexion.php");
                $query="SELECT (SELECT COUNT(*) FROM $db"."_programa_consolidado WHERE Semana=$semana AND Categoria='propias' AND Dias_Inicio>7 AND Dias_Inicio<=Lookahead AND Ejecutado=0) AS 'lookahead', (SELECT COUNT(*) FROM $db"."_programa_consolidado WHERE Semana=$semana AND Categoria='propias' AND Dias_Inicio<=7 AND Ejecutado=0) AS 'no_iniciadas', (SELECT COUNT(*) FROM $db"."_programa_consolidado WHERE Semana=$semana AND Categoria='propias' AND Ejecutado>0 AND Ejecutado<1) AS 'en_ejecucion', (SELECT COUNT(*) FROM $db"."_programa_consolidado WHERE Semana=$semana AND Categoria='propias' AND Ejecutado=1) AS 'terminadas', (SELECT COUNT(*) FROM $db"."_programa_consolidado WHERE Semana=$semana AND Categoria='propias' AND (Dias_Inicio<=Lookahead OR Ejecutado>0)) AS 'total'";
                $resultado= mysqli_query($conexion, $query);
                $data=mysqli_fetch_assoc($resultado);
...
//                "responsive":true,
                ...
                "ajax":{
                  "method":"POST",
                  "url":"../tareas_propias/listar_tareas_propias.php<?php echo "?db=$db&semana=$semana"?>&activa_lookahead="+activa_lookahead+"&activa_no_iniciadas="+activa_no_iniciadas+"&activa_en_ejecucion="+activa_en_ejecucion+"&activa_terminadas="+activa_terminadas
              },
        ...
        var obtener_data_editar=function(tbody, table){
            ...
            $(tbody).on("click", "td", function(){
                    ...
                    if(only_once==true){
                        ...
                        $("#input_Relevancia_Editar").focus();
                        $("#input_Relevancia_Editar").select();
