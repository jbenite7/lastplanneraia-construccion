...
$db=$_GET['db']/*"proyectos_inmobiliarios"*/;
...
if ($_SERVER['REQUEST_METHOD']== 'POST' && $opcion == "registrar"){
   ...
   if(empty($_POST['Confirmar_Correo'])){
    ...
    if(empty($Subcontratista) or  empty($email) or empty($confirmar_correo) or empty($NIT) or empty($alcance) /*or empty($tipo_proveedor)*/){
        $errores .='Debe rellenar todos los campos';
        $resultado=false;
        
    } else {
        $query= "SELECT * FROM $db"."_subcontratistas WHERE correo_contacto = '$confirmar_correo' OR correo_contacto = '$email' LIMIT 1";
    ...
    $f_inicio_sem=/*date("Y-m-d",strtotime("2020-03-10"));*/date("Y-m-d",strtotime($_POST["f_inicio_sem"]));
