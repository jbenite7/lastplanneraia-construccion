            ...
            <nav class="navbar navbar-expand-m navbar-dark ">
              <a class="navbar-brand" href="#">
                <ul>
                    ...
                    <li class="lps">LPS</li>
                    <li class="pagina"> - Trámites / Semana <?php echo $semana ?></li>
                </ul>
              </a>
                                    ...
                                    if(($Max_Semana-2)>=$i){
                                        ...
                                        <li class='nav-item dropdown active' style='padding: 2px 16px'>
                                            ...
                                            <div class='dropdown-menu show' aria-labelledby='navbarDropdown'>
                                                ...
                                                <a class='dropdown-item active' href='../cambiar_pagina.php?seccion=tramites&semana=$i'>Trámites</a>
...
<!--    <script type='text/javascript'>document.oncontextmenu = function(){return false}</script>-->
	...
	<script>
        ...
        $("#btn_listar").on("click", function(){
            ...
            location.assign("../scroll_general.php?scroll="+posicion+"&seccion=tramites")
            //$("#cuadro2").hide();
            //$("#cuadro1").slideDown();
            $("#cuadro3").slideDown();
        });
		...
		var guardar = function(){
			$(".checklist").on("submit", function(e){
                ...
                if($("#opcion").val()=="modificar_checklist"){
                    ...
                    $.ajax({
                        method: "POST",
                        url: "../tramites/guardar_tramites.php?db=<?php echo $db?>",
        ...
        var listar=function(){
            ...
            if(isset($_SESSION['lookahead_tram'])){
                if($_SESSION['lookahead_tram']==1){
                    echo 1;
                }else{
                    echo 0;
                }
            ...
            if(isset($_SESSION['no_iniciadas_tram'])){
                if($_SESSION['no_iniciadas_tram']==1){
                    echo 1;
                }else{
                    echo 0;
                }
            ...
            if(isset($_SESSION['en_ejecucion_tram'])){
                if($_SESSION['en_ejecucion_tram']==1){
                    echo 1;
                }else{
                    echo 0;
                }
            ...
            if(isset($_SESSION['terminadas_tram'])){
                if($_SESSION['terminadas_tram']==1){
                    echo 1;
                }else{
                    echo 0;
                }
            ...
            <?php 
                require("../conexion.php");
                $query="SELECT (SELECT COUNT(*) FROM $db"."_programa_consolidado WHERE Semana=$semana AND Categoria='tramites' AND Dias_Inicio>7 AND Dias_Inicio<=Lookahead AND Ejecutado=0) AS 'lookahead', (SELECT COUNT(*) FROM $db"."_programa_consolidado WHERE Semana=$semana AND Categoria='tramites' AND Dias_Inicio<=7 AND Ejecutado=0) AS 'no_iniciadas', (SELECT COUNT(*) FROM $db"."_programa_consolidado WHERE Semana=$semana AND Categoria='tramites' AND Ejecutado>0 AND Ejecutado<1) AS 'en_ejecucion', (SELECT COUNT(*) FROM $db"."_programa_consolidado WHERE Semana=$semana AND Categoria='tramites' AND Ejecutado=1) AS 'terminadas', (SELECT COUNT(*) FROM $db"."_programa_consolidado WHERE Semana=$semana AND Categoria='tramites' AND (Dias_Inicio<=Lookahead OR Ejecutado>0)) AS 'total'";
                $resultado= mysqli_query($conexion, $query);
                $data=mysqli_fetch_assoc($resultado);
    ...
    //                "responsive":true,
                    ...
                    "ajax":{
                      "method":"POST",
                      "url":"../tramites/listar_tramites.php<?php echo "?db=$db&semana=$semana"?>&activa_lookahead="+activa_lookahead+"&activa_no_iniciadas="+activa_no_iniciadas+"&activa_en_ejecucion="+activa_en_ejecucion+"&activa_terminadas="+activa_terminadas
                  },
        ...
        var cnc=function(causa){
            var categoria=$("#Categoria_CNC").val(),
                ...
                if(categoria===""){
                    ...
                    $.ajax({
                        method:"POST",
                        url: "../tramites/guardar_tramites.php?db=login",
                        contenttype:"charset=utf-8",
