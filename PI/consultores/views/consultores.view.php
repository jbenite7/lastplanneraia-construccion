            ...
            <nav class="navbar navbar-expand-m navbar-dark ">
              <a class="navbar-brand" href="#">
                <ul>
                    ...
                    <li class="lps">LPS</li>
                    <li class="pagina"> - Consultores / Semana <?php echo $semana ?></li>
                </ul>
              </a>
                                    ...
                                    if(($Max_Semana-2)>=$i){
                                        ...
                                        <li class='nav-item dropdown active' style='padding: 2px 16px'>
                                            ...
                                            <div class='dropdown-menu show' aria-labelledby='navbarDropdown'>
                                                ...
                                                <a class='dropdown-item active' href='../cambiar_pagina.php?seccion=consultores&semana=$i'>Consultores</a>
			...
			<form class="form-botones" action="" method="POST">
				...
				<div class="form-group">
					<div class="col-sm-offset-1 ">
                        ...
                        <div class="grupo_botones btn-group" role="group" aria-label="Basic example" style="padding:10; float:right; margin-left:2%; margin-right:30%">
...
<!--    <script type='text/javascript'>document.oncontextmenu = function(){return false}</script>-->
	...
	<script>
        ...
        $("#btn_listar").on("click", function(){
            ...
            location.assign("../scroll_general.php?scroll="+posicion+"&seccion=consultores")
            //$("#cuadro2").hide();
            //$("#cuadro1").slideDown();
            $("#cuadro3").slideDown();
        });
		...
		var guardar = function(){
			$(".checklist").on("submit", function(e){
                ...
                if($("#opcion").val()=="modificar_checklist"){
                    ...
                    $.ajax({
                        method: "POST",
                        url: "../consultores/guardar_consultores.php?db=<?php echo $db?>",
        ...
        var listar=function(){
            ...
            if(isset($_SESSION['lookahead_cons'])){
                if($_SESSION['lookahead_cons']==1){
                    echo 1;
                }else{
                    echo 0;
                }
            ...
            if(isset($_SESSION['no_iniciadas_cons'])){
                if($_SESSION['no_iniciadas_cons']==1){
                    echo 1;
                }else{
                    echo 0;
                }
            ...
            if(isset($_SESSION['en_ejecucion_cons'])){
                if($_SESSION['en_ejecucion_cons']==1){
                    echo 1;
                }else{
                    echo 0;
                }
            ...
            if(isset($_SESSION['terminadas_cons'])){
                if($_SESSION['terminadas_cons']==1){
                    echo 1;
                }else{
                    echo 0;
                }
            ...
            <?php 
                require("../conexion.php");
                $query="SELECT (SELECT COUNT(*) FROM $db"."_programa_consolidado WHERE Semana=$semana AND Categoria='consultores' AND Dias_Inicio>7 AND Dias_Inicio<=Lookahead AND Ejecutado=0) AS 'lookahead', (SELECT COUNT(*) FROM $db"."_programa_consolidado WHERE Semana=$semana AND Categoria='consultores' AND Dias_Inicio<=7 AND Ejecutado=0) AS 'no_iniciadas', (SELECT COUNT(*) FROM $db"."_programa_consolidado WHERE Semana=$semana AND Categoria='consultores' AND Ejecutado>0 AND Ejecutado<1) AS 'en_ejecucion', (SELECT COUNT(*) FROM $db"."_programa_consolidado WHERE Semana=$semana AND Categoria='consultores' AND Ejecutado=1) AS 'terminadas', (SELECT COUNT(*) FROM $db"."_programa_consolidado WHERE Semana=$semana AND Categoria='consultores' AND (Dias_Inicio<=Lookahead OR Ejecutado>0)) AS 'total'";
                $resultado= mysqli_query($conexion, $query);
                $data=mysqli_fetch_assoc($resultado);
    ...
    //                "responsive":true,
                    ...
                    "ajax":{
                      "method":"POST",
                      "url":"../consultores/listar_consultores.php<?php echo "?db=$db&semana=$semana"?>&activa_lookahead="+activa_lookahead+"&activa_no_iniciadas="+activa_no_iniciadas+"&activa_en_ejecucion="+activa_en_ejecucion+"&activa_terminadas="+activa_terminadas
                  },
        ...
        var cnc=function(causa){
            var categoria=$("#Categoria_CNC").val(),
                ...
                if(categoria===""){
                    ...
                    $.ajax({
                        method:"POST",
                        url: "../consultores/guardar_consultores.php?db=login",
                        contenttype:"charset=utf-8",
