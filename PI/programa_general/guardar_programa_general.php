...
switch($opcion){
    case 'modificar':
        modificar($Ejecutado, $Id, $semana, fecha_inicio_sem($semana, $db, $conexion), $db, $conexion);
        break;
...
function modificar($Ejecutado, $Id, $semana, $f_inicio_sem, $db, $conexion){
    //$query = "UPDATE $db"."_programa_consolidado SET Activa=1 WHERE Consecutivo_en_Programa='$Id' AND Semana=$semana;";
    $query1 = "UPDATE $db"."_programa_consolidado SET Ejecutado=$Ejecutado WHERE Consecutivo_en_Programa='$Id' AND Semana=$semana";
    //echo $query1;
