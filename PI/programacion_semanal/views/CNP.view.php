            ...
            <nav class="navbar navbar-expand-m navbar-dark ">
              <a class="navbar-brand" href="#">
                <ul>
                    ...
                    <li class="lps">LPS</li>
                    <li class="pagina"> - Evaluación Semanal / Causas de No Programación / Semana <?php echo $semana ?></li>
                </ul>
              </a>
...
<!--                <div style="width:40%; float:left;">-->
                    ...
                    <div class="form-group" style="width:600px; ; max-width:45%; float:left">
                        <label for="CNC" class="col-sm-8 control-label">Causa de No Programación</label>
				...
				<div class="form-group">
					<div class="col-sm-offset-1 ">
                        ...
                        <div class="grupo_botones btn-group" role="group" aria-label="Basic example" style="padding:10">
                            ...
                            <button id="btn_CNP" type="button" class="btn btn-success btn-sm active" onclick="window.location.href='<?php echo "../cambiar_pagina.php?seccion=CNP&semana=$semana"?>'">Causas No Programación <i class="fas fa-arrow-right fa-m"></i></button>
...
<!--						<div class="modal-footer">
            ...
            <div>     
                <form id="frmEliminarUsuario" action="" method="POST">
                    <input type="hidden" id="Id" name="Id" value="" readonly>
                    <input type="hidden" id="opcion" name="opcion" value="eliminar" readonly>
                    <input type="hidden" id="semana" name="semana" value="eliminar" readonly>
...
<!--    <script type='text/javascript'>document.oncontextmenu = function(){return false}</script>-->
	...
	<script>
        ...
        $("#btn_listar").on("click", function(){
            ...
            location.assign("../scroll_general.php?scroll="+posicion+"&seccion=CNP")
		...
		var guardar = function(){
			$("#btn_guardar1").on("click", function(e){
                ...
                frm=Id+"&"+semana+"&"+opcion+"&"+Responsable_AIA+"&"+CNC+"&"+Observaciones_CNC;
                console.log(frm);
				$.ajax({
					method: "POST",
					url: "../programacion_semanal/guardar_CNP.php?db=<?php echo $db?>",
        ...
        var reprogramar = function(){
			$("#reprogramar-usuario").on("click", function(){
				...
				$.ajax({
					...
					url: "../programacion_semanal/guardar_CNP.php?db=<?php echo $db?>",
                    contenttype:"charset=utf-8",
					data: {"Id": Id, "semana": semana, "opcion": opcion}
				}).done( function( info ){
					/*var json_info = JSON.parse( info );
					mostrar_mensaje( json_info );
					limpiar_datos();
    ...
    //                "responsive":true,
                    ...
                    "ajax":{
                      "method":"POST",
                      "url":"../programacion_semanal/listar_CNP.php<?php echo "?db=$db&semana=$semana"?>"
                  },
                    ...
                    "columns":[
                        {"defaultContent":"<button type= 'button' class='editar btn btn-primary btn-sm' style='margin:1px'><i class='fa fa-edit fa-xs'></i></button><button type= 'button' class='reprogramar btn btn-success btn-sm' style='margin:1px' data-toggle='modal' data-target='#modalReprogramar'><i class='fa fa-undo-alt fa-xs'></i></button>"},
...
//            $("#cuadro1").slideUp("slow");   
                ...
                if(only_once==true)try {
                    {
                        ...
                        var codigo_html_CNC = "<input id='select_CNC' name='CNC' class='form-control' type='text' value='"+data.CNP+"'></input>";
                        ...
                        cnc(data.CNP);
                        //console.log("<option value='"+data.CNP+"'>"+data.CNP+"</option>");
                        ...
                        $(".input_Responsable_AIA, .input_Categoria_CNC, .input_CNC, .input_Observaciones_CNC").keypress(function(e){
                            if(e.keyCode==13){
                                $("#btn_guardar1").click();
                            }
                        });
                        $(".input_Responsable_AIA, .input_Categoria_CNC, .input_CNC, .input_Observaciones_CNC").keyup(function(e){
                            if(e.keyCode==27){
                                $("#btn_cancelar2").click();
                            }
                        });
        ...
        var obtener_id_duplicar=function(tbody, table){
            $(tbody).on("click", "button.reprogramar", function(){
                var data= table.row($(this).parents("tr")).data();
                var idusuario=$("#frmEliminarUsuario #Id").val(data.Consecutivo);
                var semana=$("#frmEliminarUsuario #semana").val(data.Semana);
                var opcion=$("#frmEliminarUsuario #opcion").val("reprogramar");
                var texto=$("#modal-body-texto-reprogramar").text("¿Desea reprogramar la actividad: "+data.Actividad+"?");
            });
        }
