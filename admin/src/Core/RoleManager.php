...
class RoleManager
{
    private static $roles = [
        ...
        'P'   => ['name' => 'Planeación', 'color' => 'indigo', 'description' => 'Programación y control.'],
        'V'   => ['name' => 'Visualizador', 'color' => 'dark', 'description' => 'Acceso de solo lectura.'],
        'C'   => ['name' => 'Subcontratista', 'color' => 'orange', 'description' => 'Acceso limitado para terceros.']
    ];

    public static function getAll() { return self::$roles; }
    public static function getRoleName($code) { return self::$roles[$code]['name'] ?? 'Desconocido'; }
    public static function getRoleColor($code) { return self::$roles[$code]['color'] ?? 'light'; }
    ...
    public static function suggestRoleByCargo($cargo)
    {
        ...
        if ($learned) return $learned['suggested_role'];
        ...
        $c = $cleanCargo;
        if (str_contains($c, 'sst') && str_contains($c, 'ambiental')) return 'SG';
        if (str_contains($c, 'sst')) return 'S';
        if (str_contains($c, 'ambiental')) return 'G';
        if (str_contains($c, 'costo') || str_contains($c, 'compra') || str_contains($c, 'licitaci') || str_contains($c, 'aprovisionamiento')) return 'OT';
        if (str_contains($c, 'dcv')) return 'DCV';
        if (str_contains($c, 'planeaci') || str_contains($c, 'programaci') || str_contains($c, 'control')) return 'P';
        
        if (str_contains($c, 'director') || str_contains($c, 'gerente') || str_contains($c, 'jefe')) return 'A';
        if (str_contains($c, 'residente')) return 'R';
        if (str_contains($c, 'interventor') || str_contains($c, 'coordinador') || str_contains($c, 'invitado') || str_contains($c, 'vp')) return 'V';
    ...
    {
        $cleanCargo = self::cleanCargo($cargo);
        if (empty($cleanCargo) || empty($role)) return false;
