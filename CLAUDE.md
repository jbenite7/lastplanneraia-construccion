...
- **Este repo (`plan-de-compras`):** SPA **React + Vite + AG Grid Community** (MIT — no usar features Enterprise ni Handsontable, cuyo tier gratis es solo no-comercial). Vistas: importar presupuesto, maestro de insumos, Pareto, paquetes, plan final. Recibe contexto por `window.__PDC_BOOTSTRAP__` (project_id, rol, CSRF, base URL) y consume los tokens `aia-*` de lps-aia. El build (`dist/`) se despliega a `lps-aia/public/plan-compras/`.
- **En `../lps-aia` (glue PHP):** vista shell `views/plan-compras/app.view.php` tras `SessionMiddleware`; rutas y **endpoints JSON delgados** (`/pdc/api/...`) vía FastRoute con CSRF + RBAC; import de Excel con `phpoffice/phpspreadsheet`; tablas nuevas aisladas por `project_id` con migraciones en `database/migrations/` (ver `docs/global-tables-architecture.md`).
...
- **Deploy:** rutina de lps-aia (`docs/siteground-deploy-routine.md`). Watch-item: `memory_limit` de PhpSpreadsheet con presupuestos grandes.
